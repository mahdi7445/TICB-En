    ...
    for upd in result.get("result", []):
        ...
        if "channel_post" in upd:
            ...
            if ALTCOIN_SOURCE_CHANNEL_ID and src_chat_id == ALTCOIN_SOURCE_CHANNEL_ID:
                handle_altcoin_relay_post(post, relay_seen_ids)
            else:
...
DAILY_REPORT_RELAY_SKIP_RE = re.compile(r"Daily\s*Report", re.IGNORECASE)


def handle_altcoin_relay_post(post, relay_seen_ids):
    """
    ...
    try:
        copy_result = tg("copyMessage", chat_id=PRIVATE_CHANNEL_ID, from_chat_id=chat_id, message_id=message_id)
    except Exception as e:
