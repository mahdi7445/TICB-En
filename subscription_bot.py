...
def format_results_message():
    ...
    by_tf = compute_stats_by_group(history, "tf")
...
def is_forwarded_message(post):
    """
    تشخیص اینکه این پیام واقعاً فوروارد شده (نه تایپ مستقیم توی کانال)، با تلگرام هم روی
    فیلد جدید forward_origin (Bot API 7+) و هم فیلدهای قدیمی‌تر (forward_date/forward_from/
    forward_from_chat/forward_sender_name) که هنوز روی بعضی نسخه‌ها برمی‌گردن.
    """
    return bool(
...
def parse_forwarded_signal_text(text):
    """
    اگه متنِ فوروارد‌شده دقیقاً ساختار پیام سیگنال خودمون رو داشته باشه (LONG/SHORT + نماد
    BTC/ETH + خط Entry: + خط Stop:)، (symbol, side, entry, sl) رو برمی‌گردونه؛ وگرنه None
    (یعنی این فوروارد یه سیگنال نیست — مثلاً خبر یا پیام عادیه — و باید دست‌نخورده رد بشه).
    """
    if not text:
        return None
...
def handle_channel_signal_post(post):
    ...
    ۲) تشخیص خودکار فوروارد — چون فوروارد شما دقیقاً فرمت پیام‌های خودِ این کانال رو داره
       (🟢/🔴 LONG/SHORT، خط Entry:، خط Stop:)، نیازی به تگ نیست: هر پیامی که واقعاً «فوروارد
       شده» باشه (بر اساس متادیتای forward تلگرام، نه صرفاً شکل ظاهری) و این ساختار رو داشته
       باشه، خودکار پارس و صف می‌شه. فوروادهای بی‌ربط (خبر، پیام عادی) که این ساختار رو
       ندارن ساکت رد می‌شن — چیزی توی کانال پست نمی‌شه.
    ...
    # مسیر ۲: پیامِ واقعاً فوروارد‌شده‌ای که فرمت سیگنال رو داره
    if is_forwarded_message(post):
        parsed = parse_forwarded_signal_text(text)
        if not parsed:
            return  # فوروارد بی‌ربط؛ کاری نمی‌کنیم
        ...
        send_message(chat["id"], "🔎 Detected as a forwarded signal.\n\n" + note, reply_to_message_id=message_id)
        return
