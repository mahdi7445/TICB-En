...
# ⚠️ این وزن‌ها باید همیشه دقیقاً با candle_engine.py (W1/W2/W3/W4/W_RUNNER و RR_TARGETS)
# یکی بمونن - عمداً این‌جا هم تکرار شدن (به‌جای import از یک فایل مشترک) چون این دو اسکریپت
# دو ورک‌فلوی کاملاً جدا و مستقل هستن. اگه یکی رو عوض کردید، اون یکی رو هم عوض کنید.
_RR_TARGETS = [1, 2, 4, 6]
_W1, _W2, _W3, _W4, _W_RUNNER = 0.20, 0.30, 0.15, 0.10, 0.25


def _live_r(trade):
    """همون منطق compute_final_r در candle_engine.py، برای نمایش زنده‌ی «نتیجه‌ی بانک‌شده
    تا این لحظه» روی معاملات هنوز بازِ candle_state.json."""
    hit = trade.get("hit", {})
    if not hit.get("1"):
        return -1.0
    if not hit.get("2"):
        return _W1 * 1 + (1 - _W1) * 0.0
    if not hit.get("4"):
        return _W1 * 1 + _W2 * 2 + (1 - _W1 - _W2) * 1.0
    if not hit.get("6"):
        return _W1 * 1 + _W2 * 2 + _W3 * 4 + (1 - _W1 - _W2 - _W3) * 2.0
    return _W1 * 1 + _W2 * 2 + _W3 * 4 + _W4 * 6 + _W_RUNNER * 6.0
...
def format_open_trades_section():
    ...
    for key, t in open_trades:
        ...
        symbol_part = t.get("symbol", "?") if tf_key == "manual" else f"{t.get('symbol', '?')} · {tf_key}"
...
def format_results_message():
    ...
    # سیگنال‌های دستی/فوروارد‌شده (tf == "manual") یک تایم‌فریمِ واقعی ندارن، پس توی این
    # جدولِ «بر اساس تایم‌فریم» نمی‌آن (وگرنه انگار «manual» یک ارز/دسته‌ی جدا کنار BTC/ETH
    # بود). این سیگنال‌ها همچنان کامل توی جمعِ کلی بالا و توی «By symbol» (با نام خودِ همون
    # ارز) حساب می‌شن - فقط از این breakdown خاص جدا نگه داشته می‌شن.
    tf_history = [h for h in history if h.get("tf") != "manual"]
    by_tf = compute_stats_by_group(tf_history, "tf")
...
def handle_updates(state, pending, subscribers, discounts, applied, used_tx, referrals, maintenance, photo_submissions):
    ...
    for upd in result.get("result", []):
        ...
        if "channel_post" in upd:
            handle_channel_signal_post(upd["channel_post"])
            continue
        ...
        if "edited_channel_post" in upd:
            handle_channel_signal_post(upd["edited_channel_post"])
            continue
...
ADMIN_HELP = (
    ...
    "<u>Manual signals</u>\n"
    "/admin_manual_signal &lt;BTC/USD|ETH/USD&gt; &lt;BUY|SELL&gt; &lt;entry&gt; &lt;sl&gt; — post a "
    ...
    "— same tracking, same /results. Untagged posts are left alone.\n\n"
    "<u>Photo rewards</u>\n"
...
def queue_manual_signal(symbol, side, entry_s, sl_s):
    """
    ...
    (در چت خصوصی با ربات) و پست مستقیم در کانال با تگ #signal.
    """
    symbol = (symbol or "").upper()
    side = (side or "").upper()
    if symbol not in ("BTC/USD", "ETH/USD"):
        return False, "❌ Symbol must be BTC/USD or ETH/USD."
    ...
    return True, (
        ...
        f"signals) within a few minutes by the candle engine, and its result will "
        f"count in /results."
    )


CHANNEL_SIGNAL_TAG_RE = re.compile(
    r"#signal\s+(BTC/USD|ETH/USD)\s+(BUY|SELL)\s+([\d.]+)\s+([\d.]+)",
    re.IGNORECASE,
)

# فرمتی که خود candle_engine.py برای پیام سیگنال جدید می‌سازه (format_entry_message):
#   🟢 LONG — BTC 1h            یا           🔴 SHORT — ETH 15m
#
#   Entry: 65000.00
#   ❌ Stop: 64000.00
#   🎯 Target 1: ...
...
# سه چیز (LONG/SHORT، نماد BTC/ETH، Entry:، Stop:) استخراج می‌کنیم.
FORWARDED_SIDE_RE = re.compile(r"\b(LONG|SHORT)\b", re.IGNORECASE)
FORWARDED_SYMBOL_RE = re.compile(r"\b(BTC|ETH)\b", re.IGNORECASE)
FORWARDED_ENTRY_RE = re.compile(r"Entry:\s*\$?([\d,]+\.?\d*)", re.IGNORECASE)
FORWARDED_STOP_RE = re.compile(r"Stop:\s*\$?([\d,]+\.?\d*)", re.IGNORECASE)
...
def parse_forwarded_signal_text(text):
    ...
    شده، ساختار سیگنال رو استخراج می‌کنه: LONG/SHORT + نماد BTC/ETH + خط Entry: + خط
    Stop:. اگه هر کدوم پیدا نشه None برمی‌گردونه (یعنی با این‌که با ایموجی شروع شده، ساختار
    کامل سیگنال رو نداره - پس دست‌نخورده رد می‌شه، نه اینکه با داده‌ی ناقص صف بشه).
    """
    if not text:
        return None
    side_m = FORWARDED_SIDE_RE.search(text)
    symbol_m = FORWARDED_SYMBOL_RE.search(text)
    entry_m = FORWARDED_ENTRY_RE.search(text)
    stop_m = FORWARDED_STOP_RE.search(text)
    if not (side_m and symbol_m and entry_m and stop_m):
        return None

    side = "BUY" if side_m.group(1).upper() == "LONG" else "SELL"
    symbol = "BTC/USD" if symbol_m.group(1).upper() == "BTC" else "ETH/USD"
    try:
        entry = float(entry_m.group(1).replace(",", ""))
        sl = float(stop_m.group(1).replace(",", ""))
    except ValueError:
        return None
    return symbol, side, entry, sl
...
def handle_channel_signal_post(post):
    ...
    ۲) تشخیص خودکار بر اساس محتوا — هر پیامی (فوروارد شده یا مستقیم پیست/تایپ‌شده، فرقی
       ...
       داشته باشه (خط Entry:، خط Stop:)، خودکار به‌عنوان یکی از سیگنال‌های ربات شناخته و صف
       می‌شه - دقیقاً مثل بقیه‌ی سیگنال‌ها: چارت و پیام تارگت/استاپ منتشر می‌شه، و در نتایج
       با نام خودِ همون ارز (نه یک برچسب جدا) حساب می‌شه. پیام‌هایی که این ایموجی رو ندارن
       (اعلامیه، فوروارد بی‌ربط) دست‌نخورده رد می‌شن.

    هر دو مسیر دقیقاً از همون تابع queue_manual_signal (مسیر /admin_manual_signal) استفاده
    می‌کنن، پس نتیجه یکسانه: پست استاندارد با چارت + ۴ تارگت، رصد زنده، و شمارش در /results.
    """
    ...
    if starts_with_signal_emoji(text):
        ...
        if not parsed:
            send_message(
                chat["id"],
                "⚠️ This starts with the signal emoji but I couldn't find both an "
                "<code>Entry:</code> line and a <code>Stop:</code> line with a clear "
                "BTC/ETH + LONG/SHORT — skipped. Use <code>#signal BTC/USD BUY 65000 64000</code> "
                "if you want to enter it manually.",
                reply_to_message_id=message_id,
            )
            return
        symbol, side, entry, sl = parsed
        if is_duplicate_signal(symbol, side, entry, sl):
            ...
            return
        source_note = "forwarded" if is_forwarded_message(post) else "posted"
        ok, note = queue_manual_signal(symbol, side, str(entry), str(sl))
        send_message(chat["id"], f"🔎 Detected as a {source_note} signal.\n\n" + note, reply_to_message_id=message_id)
        return
...
def handle_admin_command(chat_id, text, subscribers, discounts, maintenance, photo_submissions):
    ...
    if cmd == "/admin_manual_signal":
        if len(parts) != 5:
            send_message(chat_id, "Usage: /admin_manual_signal <BTC/USD|ETH/USD> <BUY|SELL> <entry> <sl>")
            return
...
def git_commit_and_push():
    """هر چند دقیقه یک‌بار تغییرات data/ رو کامیت و پوش می‌کنه، و - مهم - همیشه یک `git pull`
    هم انجام می‌ده حتی اگه خودِ این اسکریپت چیزی برای کامیت نداشته باشه.

    ⚠️ باگ قبلی: pull فقط زمانی اجرا می‌شد که این اسکریپت خودش تغییر محلی برای کامیت داشت.
    یعنی trade_history.json و manual_signals.json (که candle_engine.py - یک workflow کاملاً
    جدا با چک‌اوت گیت جداگانه - می‌نویسه) توی کل عمر این اجرای طولانی (تا ۵ ساعت و ۲۰ دقیقه)
    اصلاً pull نمی‌شدن، مگر این‌که این اسکریپت به‌طور تصادفی خودش هم چیزی برای کامیت داشته
    باشه. نتیجه: /results و /pnl می‌تونستن تا همون مدت با تاخیر/کهنه باشن. الان با pull
    بی‌قید‌وشرط در هر دور، این تاخیر به حداکثر GIT_COMMIT_EVERY_SECONDS (چند دقیقه) کاهش
    پیدا می‌کنه."""
    try:
        import subprocess
        ...
        if pull.returncode != 0:
            print("[warn] git pull --rebase failed/conflict — will retry next cycle")
        if has_local_changes:
            subprocess.run(["git", "push"], cwd=repo_dir, check=True)
            print("[git] committed & pushed data/ changes")
        elif pull.returncode == 0:
            print("[git] pulled latest data/ (no local changes to push)")
    except Exception as e:
...
def main():
    ...
    while time.time() - start < LOOP_MAX_SECONDS:
        try:
            run_cycle(state, pending, subscribers, discounts, applied, used_tx, referrals, maintenance, photo_submissions, timers)
        except Exception as e:
