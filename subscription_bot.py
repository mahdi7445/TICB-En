...
def git_commit_and_push():
    ...
    اسکریپت و candle_engine.py، دقیقاً به همون دلیلی که shared_risk_config.py جدا شد: این
    منطق قبلاً عیناً و دستی توی هر دو فایل کپی شده بود.

    ⚠️ چرا الان retry فوری اضافه شد (علت خطای «git push failed ... Updates were rejected»):
    این اسکریپت و candle_engine.py هرکدوم در یک workflow/checkout کاملاً جدا، به‌صورت پیوسته
    برای ~۵ ساعت و ۲۰ دقیقه، به همون شاخه push می‌کنن (این هر ۱۲۰ ثانیه، اون هر ۴۵ ثانیه).
    روی این تعداد push، برخورد (non-fast-forward) قطعاً پیش میاد - اتفاق عادی این معماریه.
    قبلاً retry فقط توی «دور بعدی» انجام می‌شد؛ الان shared_git_sync همون‌جا تا ۴ بار با
    تاخیر تصادفی retry می‌کنه، پس اکثر برخوردها اصلاً به alert_admins_text نمی‌رسن."""
    try:
        ...
        def on_error(kind: str, message: str):
            ...
            alert_admins_text(f"⚠️ subscription_bot.py ({kind}): {message}")
