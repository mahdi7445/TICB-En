...
def _ensure_fresh_data() -> None:
    ...
    نسخه‌ی محلیِ ساعت‌ها قدیمی‌تر ساخته شده باشد (مثلاً از قبل از اینکه یک سیگنال اصلاً باز
    شده باشد) - دقیقاً همون حسِ «این سیگنال انگار اصلاً وجود نداشته». الان اگه pull شکست
    بخوره، به ادمین (نه به کاربر - تا اسپم نشه) یک هشدار واضح می‌ره، تا این حالت به‌جای
    بی‌صدا-همیشه-قابل‌مشاهده بودن، قابل ردیابی و رفع بشه."""
    try:
        ok = pull_latest_readonly(os.path.dirname(os.path.abspath(__file__)))
        if not ok:
            print("[warn] pre-/results pull did not succeed (serving last-known-local data)")
            alert_admins_text(
                ...
                "opened/closed signals). This usually resolves itself on the next successful "
                "sync, but if it keeps happening, check for a persistent git conflict."
            )
    except Exception as e:
