...
def git_commit_and_push():
    ...
    ⚠️ باگ دوم (تازه پیدا‌شده، هم‌ریشه با همون باگ توی candle_engine.py که باعث «سیگنال توی
    کانال هست ولی توی /results نیست» می‌شد): اگه `git pull --rebase` شکست می‌خورد، فقط warning
    چاپ می‌شد و ریپو توی حالت نیمه‌rebase می‌موند؛ و push فقط وقتی امتحان می‌شد که همین دور
    خودش commit جدید داشت - یعنی یک push ناموفق ممکن بود برای همیشه بدون retry بمونه. حالا
    rebase ناموفق صراحتاً abort می‌شه، push همیشه (نه فقط با تغییر محلیِ همین دور) امتحان
    می‌شه، و هر شکستی مستقیم به ادمین‌ها پیام می‌ده."""
    try:
        import subprocess
        ...
        if has_local_changes:
            run(["git", "commit", "-m", "update subscription data [skip ci]"], check=True)

        pull = run(["git", "pull", "--rebase"])
        if pull.returncode != 0:
            run(["git", "rebase", "--abort"])
            print("[warn] git pull --rebase failed/conflict — aborted, will retry next cycle")
            alert_admins_text(
                "⚠️ subscription_bot.py: git pull --rebase failed and was aborted. Local changes "
                "(if any this cycle) are committed but not pushed yet — will retry automatically "
                "next cycle. If this keeps repeating, there's likely a real conflict in data/ "
                "needing a manual look."
            )
            return

        # همیشه push رو امتحان کن - نه فقط وقتی همین دور خودش چیزی برای commit داشت (وگرنه یک
        # push ناموفق قبلی می‌تونه برای همیشه بدون retry بمونه - علت اصلی گزارش «سیگنال توی
        # کانال هست ولی توی /results نیست»، حتی اگه ریشه‌ی خودِ اون مورد سمت candle_engine.py
        # بوده باشه، این اسکریپت هم دقیقاً همین ضعف رو داشت).
        push = run(["git", "push"])
        if push.returncode != 0:
            print("[warn] git push failed — will retry next cycle")
            alert_admins_text(
                "⚠️ subscription_bot.py: git push failed. Local data changes will NOT be visible "
                "to candle_engine.py until this succeeds — will keep retrying automatically."
            )
