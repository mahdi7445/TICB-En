...
ADMIN_HELP = (
    ...
    "<u>Stuck trades</u>\n"
    "/admin_close_trade &lt;key&gt; [final_r] — force-close any stuck open trade (key from a "
    "stale_open_trade alert). Always works, regardless of the underlying cause.\n\n"
    "<u>Pricing</u>\n"
...
def handle_admin_command(chat_id, text, subscribers, discounts, maintenance, photo_submissions,
    ...
    if cmd == "/admin_close_trade":
        ...
        # کد): key دقیقاً همون چیزیه که در هشدار stale_open_trade (context=...) اومده،
        # مثلاً «BTC/USD|4h» یا «MANUAL|m-1787774585». final_r اختیاریه (پیش‌فرض ۰ - یعنی نه
        # سود نه ضرر)؛ اگه ادمین می‌دونه واقعاً به کجا رسیده (مثلاً دستی چک کرده)، می‌تونه
        # عدد دقیق رو هم بده تا آمار trade_history درست بمونه.
        if len(parts) not in (2, 3):
            send_message(chat_id, "Usage: /admin_close_trade <key> [final_r]\n"
                                   "key comes from a stale_open_trade alert, e.g. BTC/USD|4h or MANUAL|m-1787774585\n"
                                   "final_r defaults to 0 (breakeven) if not given.")
            return
        key = parts[1]
        final_r = None
        if len(parts) == 3:
        ...
        # 🔴 اضافه شد: قبلاً این فایل هیچ سقفی نداشت (برخلاف forwarded_results_queue.json که
        # همین‌جا با [-500:] محدود می‌شه) - یعنی هر /admin_close_trade که تا امروز صادر شده
        # (applied/not_found/pending) برای همیشه توی فایل می‌موند و فقط بزرگ‌تر می‌شد. چون
        # این دستور دستی و کم‌تعداده، به‌خودی‌خود مشکل فوری نبود، ولی روی یک سیستم ۲۴/۷ که
        # قراره ماه‌ها/سال‌ها بدون توقف کار کنه، نگه‌داشتنِ سقف مشابه بقیه‌ی صف‌ها بی‌هزینه و
        # درسته - رکوردهای واقعاً قدیمی (applied/not_found از هفته‌ها پیش) هیچ ارزشی به‌جز
        # آرشیو ندارن که trade_history.json/گزارش‌ها از قبل پوشش می‌دن.
        requests_ = requests_[-500:]
        _save("admin_close_requests.json", requests_)
        send_message(chat_id, f"✅ Queued — candle_engine.py will close '{key}' (final_r="
                               f"{final_r if final_r is not None else 0.0}) within a few seconds. "
                               f"You'll get an alert if the key wasn't found.")
        return
