...
def queue_manual_signal(symbol, side, entry_s, sl_s, forwarded_tracking=False, tf_label=None):
    """
...
def handle_channel_signal_post(post):
    ...
    if starts_with_signal_emoji(text):
        ...
        source_note = "forwarded" if forwarded else "posted"
...
def git_commit_and_push():
    ...
    یعنی trade_history.json و manual_signals.json (که candle_engine.py می‌نویسه) توی کل عمر
    این اجرای طولانی دیده نمی‌شدن مگر این اسکریپت خودش هم چیزی commit می‌کرد. رفع شد: pull
    هر دور بی‌قید‌وشرط اجرا می‌شه.

    ⚠️ باگ دوم (هم‌ریشه با همون باگ توی candle_engine.py که باعث «سیگنال توی کانال هست ولی
    توی /results نیست» می‌شد): اگه `git pull --rebase` شکست می‌خورد، ریپو توی حالت نیمه‌rebase
    می‌موند، و push فقط وقتی امتحان می‌شد که همین دور خودش commit جدید داشت - یعنی یک push
    ناموفق می‌تونست برای همیشه بدون retry بمونه. رفع شد: rebase ناموفق صراحتاً abort می‌شه، و
    push همیشه (نه فقط با تغییر محلیِ همین دور) امتحان می‌شه.

    ⚠️ باگ سوم (علت خطای دیده‌شده در تلگرام: «git add ... returned non-zero exit status 128»):
    exit code 128 روی `git add` معمولاً یا خطای «dubious ownership» گیت روی CI‌هاست (رایج‌ترین
    حالت - رفع پیشگیرانه با safe.directory پایین‌تر)، یا یک `.git/index.lock` جامونده، یا مسیر
    اصلاً ریپو نیست. همچنین قبلاً `check=True` بدون capture خروجی بود، یعنی پیام خطا به ادمین
    فقط کد خروجی خام رو نشون می‌داد نه دلیل واقعی‌اش - حالا با capture_output متن واقعی خطای
    گیت (stderr) توی پیام هشدار می‌ره."""
    try:
        import subprocess
        ...
        def _err_tail(res, n=400):
            txt = (res.stderr or res.stdout or "").strip()
            return txt[-n:] if txt else "(no stderr/stdout captured)"

        # رفع پیشگیرانه‌ی رایج‌ترین علت exit 128 روی گیت‌های جدید در CI: مالکیتِ پوشه‌ی
        # checkout «مشکوک» تشخیص داده می‌شه مگر صریحاً safe.directory معرفی بشه. idempotent
        # و بی‌خطر حتی اگه علت مشکل چیز دیگه‌ای باشه.
        subprocess.run(["git", "config", "--global", "--add", "safe.directory", repo_dir],
                        capture_output=True, text=True)

        if not os.path.isdir(os.path.join(repo_dir, ".git")):
            print(f"[error] {repo_dir} does not look like a git repository (.git missing) - skipping sync")
            alert_admins_text(
                f"⚠️ subscription_bot.py: {repo_dir} has no .git directory — data can't be "
                f"committed/pushed at all until the checkout is fixed."
            )
            return
        ...
        if os.path.exists(lock_file):
            try:
                os.remove(lock_file)
                print("[git] removed a stale .git/index.lock before retrying")
            except OSError as e:
                print(f"[error] found .git/index.lock but couldn't remove it: {e}")

        add_res = run(["git", "add", "data/"])
        if add_res.returncode != 0:
            print(f"[error] git add failed: {_err_tail(add_res)}")
            alert_admins_text(f"⚠️ subscription_bot.py: git add data/ failed: {_err_tail(add_res)}")
            return

        result = run(["git", "diff", "--cached", "--quiet"])
        has_local_changes = (result.returncode != 0)
        if has_local_changes:
            commit_res = run(["git", "commit", "-m", "update subscription data [skip ci]"])
            if commit_res.returncode != 0:
                print(f"[error] git commit failed: {_err_tail(commit_res)}")
                alert_admins_text(f"⚠️ subscription_bot.py: git commit failed: {_err_tail(commit_res)}")
                return

        pull = run(["git", "pull", "--rebase"])
        if pull.returncode != 0:
            run(["git", "rebase", "--abort"])
            print(f"[warn] git pull --rebase failed/conflict — aborted, will retry next cycle: {_err_tail(pull)}")
            alert_admins_text(
                f"⚠️ subscription_bot.py: git pull --rebase failed and was aborted ({_err_tail(pull)}). "
                f"Local changes (if any this cycle) are committed but not pushed yet — will retry "
                f"automatically next cycle. If this keeps repeating, there's likely a real conflict "
                f"in data/ needing a manual look."
            )
            return

        # همیشه push رو امتحان کن - نه فقط وقتی همین دور خودش چیزی برای commit داشت (وگرنه یک
        # push ناموفق قبلی می‌تونه برای همیشه بدون retry بمونه).
        push = run(["git", "push"])
        if push.returncode != 0:
            print(f"[warn] git push failed — will retry next cycle: {_err_tail(push)}")
            alert_admins_text(
                f"⚠️ subscription_bot.py: git push failed ({_err_tail(push)}). Local data changes "
                f"will NOT be visible to candle_engine.py until this succeeds — will keep retrying "
                f"automatically."
            )
        elif has_local_changes:
            print("[git] committed & pushed data/ changes")
        else:
            print("[git] pulled latest data/ (nothing new to push)")
    except Exception as e:
        print(f"[warn] git sync failed: {e}")
        try:
            alert_admins_text(f"⚠️ subscription_bot.py: git sync raised an exception: {e}")
        except Exception:
            pass
