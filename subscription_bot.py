        ...
        if time.time() - _last_pull_fail_alert_ts >= 600:
            ...
            try:
                alert_admins_text(
                    ...
                    f"(user was shown the last-known-local snapshot instead, which may be stale): {e}"
                )
            except Exception:
                pass
...
def main():
    ...
    while time.time() - start < LOOP_MAX_SECONDS:
        ...
        if time.time() - last_commit >= GIT_COMMIT_EVERY_SECONDS:
            try:
                ...
                try:
                    alert_admins_text(f"⚠️ subscription_bot.py: periodic save/commit/heartbeat failed unexpectedly: {e}")
                except Exception:
                    pass
    ...
    try:
        ...
        try:
            alert_admins_text(f"⚠️ subscription_bot.py: final save/commit (end of Job) failed unexpectedly: {e} — "
...
if __name__ == "__main__":
    ...
    try:
        ...
        try:
            alert_admins_text(f"🛑 subscription_bot.py crashed with an unhandled error and stopped: {e}\n"
