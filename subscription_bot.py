...
GUIDE_STEPS = {
    ...
    3: (
        ...
        "/signal TC-004821 — full timeline of one specific signal (shown as 🆔 in its channel message; "
...
def format_signal_lookup_message(signal_id: str) -> str:
    ...
    if t is None:
        return (
            ...
            f"Double-check the id from the channel message (the 🆔 line) or /history."
        )
            ...
            if text.startswith("/start"):
                ...
                if len(parts) != 2 or not parts[1].strip():
                    send_message(chat_id, "Usage: /signal ID — e.g. /signal TC-004821\n"
                                           "(the ID is shown in the 🆔 line of the channel message, "
...
ADMIN_HELP = (
    ...
    "🆔 line — the older symbol|timeframe key still works as a fallback). Always works, regardless "
...
# ⚠️ استخراج شناسه‌ی منبع (🆔 TC-004821 یا 🆔 ALT-000005) از متن هر پیامِ فوروارد/رله‌شده -
# چه پیام سیگنال ورودی چه پیام نتیجه (تارگت/استاپ/...) - چون _id_line در candle_engine.py
# این خط رو در هر دوی این‌ها می‌ذاره. طبق درخواست صریح («مسیر هر سیگنال را از روی هشتگ‌های
# آن دنبال و محاسبه کن، به شرطی که تحت هیچ شرایطی مسیر هیچ سیگنالی گم نشود»): این ID دقیقاً
# همون چیزیه که کاربر زیر هر پیام کانال می‌بینه - با استخراج و نگه‌داشتنش از همون لحظه‌ی
# ورودی، همه‌ی مراحل بعدی (باز شدن معامله، تطبیق نتایج فوروارد‌شده در process_forwarded_results،
# و جستجوی /signal یا /admin_close_trade) می‌تونن دقیقاً همین یک ID رو دنبال کنن - نه یک حدسِ
# جداگانه (نزدیک‌ترین قیمت/تیکر) که ممکنه اشتباه بره یا هیچ‌وقت تطبیق پیدا نکنه.
SOURCE_SIGNAL_ID_RE = re.compile(r"🆔\s*([A-Za-z]{2,10}[-_]\d{3,8})")
    ...
    if cmd == "/admin_close_trade":
        ...
        if len(parts) not in (2, 3):
            send_message(chat_id, "Usage: /admin_close_trade <signal_id or key> [final_r]\n"
                                   ...
                                   "message's 🆔 line, e.g. TC-004821 (the older key like BTC/USD|4h "
