...
def _normalize_signal_id(raw: str) -> str:
    """اجازه می‌ده کاربر شناسه رو با هر فرمتی بنویسه - «TC-004821»، «tc-4821»، یا فقط «4821» -
    و همیشه به فرمت استاندارد «TC-XXXXXX» نرمالایز می‌شه."""
    ...
    if raw.isdigit():
        return f"TC-{int(raw):06d}"
    return f"TC-{raw}" if raw else ""
