...
def handle_admin_command(chat_id, text, subscribers, discounts, maintenance, photo_submissions):
    ...
    if cmd == "/admin_manual_signal":
        ...
        if symbol not in ("BTC/USD", "ETH/USD"):
            send_message(chat_id, "Symbol must be BTC/USD or ETH/USD.")
            return
        if side not in ("BUY", "SELL"):
            send_message(chat_id, "Side must be BUY or SELL.")
            return
        try:
            entry, sl = float(entry_s), float(sl_s)
        except ValueError:
            send_message(chat_id, "entry and sl must be numbers.")
            return
        if (side == "BUY" and sl >= entry) or (side == "SELL" and sl <= entry):
            send_message(chat_id, "Stop-loss must be below entry for BUY, or above entry for SELL.")
            return
        ...
        send_message(chat_id, f"📌 Manual signal queued: {symbol} {side} entry={entry} sl={sl}\n\n"
