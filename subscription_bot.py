...
WELCOME_TEXT = (
    ...
    # 🔴 اضافه شد طبق درخواست صریح: قبلاً این متن فقط از BTC/ETH اسم می‌برد، در حالی که یک
    # کانال آلتکوین کاملاً جداگانه هم فعاله (handle_altcoin_relay_post) - یک مشترکِ تازه که
    # فقط همین متن رو می‌خونه، متوجه نمی‌شد سیگنال‌های آلتکوین هم بخشی از سرویسه.
    "Rule-based entries on <b>BTC, ETH, and top altcoins</b> — every timeframe, 1-minute to daily.\n"
...
GUIDE_STEPS = {
    ...
    2: (
        ...
        "🎯 Target 1 (1R): close 20%  ·  stop moves to entry (risk-free)\n"
        "🎯 Target 2 (2R): close 30%  ·  stop moves to Target 1\n"
        "🎯 Target 3 (4R): close 15%  ·  stop moves to Target 2\n"
        "🎯 Target 4 (6R): close 10%\n"
        "🏁 Final 25% (\"Runner\"): switches to a trailing stop 1.5R behind the peak — no fixed "
        "exit, so a bigger move keeps paying\n\n"
...
def _load(name, default):
    ...
    return read_json_resilient(path, default, label=name,
                                on_corrupt=lambda label, msg: alert_admins_text(f"⚠️ {html.escape(str(label))}: {html.escape(str(msg))}"))
...
_last_pull_fail_alert_ts = 0  # throttle برای هشدارِ زیر (نه برای خودِ pull - آن هر بار واقعی امتحان می‌شه)
...
def _ensure_fresh_data() -> None:
    ...
    🔴 رفعِ باگِ واقعیِ «نتایج/سیگنال‌ها انگار اصلاً وجود نداشته‌اند»: قبلاً وقتی همین pull
    شکست می‌خورد، تنها اثرش یک print در لاگِ GitHub Actions بود که هیچ‌کس نمی‌دید - یعنی
    کاربر یک /results کاملاً معمولی و معتبر به‌نظر می‌دید، در حالی که ممکن بود از روی یک
    نسخه‌ی محلیِ ساعت‌ها قدیمی‌تر ساخته شده باشد. الان اگه pull شکست بخوره، به ادمین یک
    هشدار می‌ره.

    🔴 اضافه شد: وقتی این شکست *مکرر/پایدار* پیش میاد (نه فقط یک بارِ گذرا)، متنِ عمومیِ قبلی
    («check for a persistent git conflict») هیچ سرنخی برای پیداکردنِ علتِ واقعی نمی‌داد - نه
    برای ادمین نه برای دیباگِ بعدی. الان detail (متنِ واقعیِ stderr گیت از pull_latest_readonly)
    مستقیم در هشدار می‌آد. و چون این هشدار می‌تونه مکرر رخ بده، برای جلوگیری از اسپم فقط هر
    ۱۰ دقیقه یک‌بار واقعاً ارسال می‌شه (خودِ تلاشِ pull هربار انجام می‌شه، فقط ارسالِ هشدار
    throttle می‌شه) - ولی چون detail الان مشخصه، یک هشدار با جزئیاتِ دقیق کافیه، نیازی به
    دیدنِ همون پیامِ یکسان ده‌ها بار نیست."""
    global _last_pull_fail_alert_ts
    try:
        ok, detail = pull_latest_readonly(os.path.dirname(os.path.abspath(__file__)))
        if not ok:
            print(f"[warn] pre-/results pull did not succeed (serving last-known-local data): {detail}")
            if time.time() - _last_pull_fail_alert_ts >= 600:
                _last_pull_fail_alert_ts = time.time()
                alert_admins_text(
                    "⚠️ subscription_bot.py: could not pull the latest data before answering a "
                    "/results, /history, /equity, /signal, or /pnl request — the user was shown "
                    "the last-known-local snapshot instead, which may be stale (missing recently "
                    "opened/closed signals).\n\n"
                    f"<b>Actual git error:</b>\n<code>{html.escape(detail)}</code>\n\n"
                    "(This alert is throttled to once per 10 minutes even if it keeps happening, "
                    "to avoid spam — check the error above for the real cause.)"
                )
    except Exception as e:
        ...
        if time.time() - _last_pull_fail_alert_ts >= 600:
            ...
            try:
                alert_admins_text(
                    f"⚠️ subscription_bot.py: pre-/results data pull raised an unexpected error "
                    f"(user was shown the last-known-local snapshot instead, which may be stale): {html.escape(str(e))}"
                )
            except Exception:
                pass
...
def load_open_trades():
    ...
    🔴 اضافه شد: مرتب‌سازی بر اساس زمانِ صدورِ سیگنال (opened_at_ms - همون فیلدی که
    candle_engine.py::open_new_trade موقع باز کردنِ هر معامله، چه خودکار چه دستی/فوروارد‌شده،
    ثبت می‌کنه). قبلاً ترتیب صرفاً همون ترتیبِ کلیدهای دیکشنریِ candle_signals در
    candle_state.json بود - که نه زمانی بود نه حتی همیشه ثابت (بستگی به ترتیبِ insert/
    بازنویسیِ کلیدها در دیکشنری داشت) - دقیقاً همون چیزی که در «Open right now» باعث می‌شد
    ترتیب نمایش هیچ ربطی به ترتیبِ واقعیِ صدور سیگنال‌ها نداشته باشه."""
    ...
    open_trades.sort(key=lambda item: item[1].get("opened_at_ms") or 0)
    return open_trades
...
def _queue_pending_manual_fix(symbol: str, side: str, entry: float, sl: float,
                               source_signal_id: Optional[str], note: str,
                               signal_message_id: Optional[int] = None) -> str:
    """وقتی یک سیگنالِ رله‌شده مخابره می‌شه ولی رهگیریش رد می‌شه (معمولاً چون تایم‌فریمِ
    منبع نامعتبر/ناشناخته بود)، جزییاتش رو اینجا با یک شناسه‌ی کوتاه ذخیره می‌کنیم - چون
    callback_data تلگرام سقفِ ۶۴ بایت داره و نمی‌شه symbol/entry/sl/source_signal_id رو
    مستقیم توی خودِ دکمه جا داد (خصوصاً برای نمادهای طولانی‌تر یا قیمت‌های اعشاریِ بلند).
    شناسه‌ی کوتاهِ برگشتی توی callback_data («mfix:{id}:{tf}») می‌ره؛ handle_callback_query
    با همین شناسه رکورد کامل رو دوباره می‌خونه.

    🔴 signal_message_id هم اینجا نگه داشته می‌شه (همون دلیلِ signal_message_id در
    queue_manual_signal - نگاه کنید توضیح آنجا) تا وقتی ادمین دکمه‌ی تایم‌فریم رو بزنه و این
    سیگنال بالاخره واقعاً صف بشه، هنوز بشه به همون پیامِ ورودیِ کپی‌شده در کانال ریپلای کرد."""
    fixes = _load("pending_manual_fixes.json", [])
    fix_id = f"f{now_ts()}"
    fixes.append({"id": fix_id, "symbol": symbol, "side": side, "entry": entry, "sl": sl,
                  "source_signal_id": source_signal_id, "note": note, "created_at": now_ts(),
                  "signal_message_id": signal_message_id})
    fixes = fixes[-200:]  # سقف - این‌ها موقتی‌ان، نیازی به آرشیو بلندمدت ندارن
    _save("pending_manual_fixes.json", fixes)
    return fix_id
...
def _last_git_commit_info() -> str:
    try:
        ...
        return f"unavailable ({html.escape(str(e))})"
...
def format_admin_status_message() -> str:
    ...
    # 🔴 تکمیلِ داشبورد: قبلاً سیگنال‌های *رد‌شده در ورودی* (manual_signals.json - همون‌هایی
    # که alert آنی‌شون در candle_engine.py::process_manual_and_forwarded_queues قبلاً throttle
    # می‌شد و اخیراً بر اساس شناسه یکتا شد) اصلاً در این داشبوردِ «یک‌نگاهی» شمارش نمی‌شدند -
    # یعنی ادمین برای دیدنشون مجبور بود دستی توی تاریخچه‌ی چت بگرده دنبال هشدار. حالا شمارشِ
    # ۲۴ساعتِ اخیر (نه کلِ تاریخچه - یک رد‌شدنِ هفته‌ی پیش دیگه «نیاز به توجهِ الان» نیست) اینجا
    # هم می‌آد.
    manual_signals = _load("manual_signals.json", [])
    cutoff_ts = now_ts() - 24 * 3600
    rejected_recent = sum(
        1 for m in manual_signals
        if m.get("status") in ("rejected_stale", "rejected_no_tf", "skipped_duplicate")
        and (m.get("created_at") or 0) >= cutoff_ts
    )
    ...
    if rejected_recent:
        attention_lines.append(f"  ⚠️ {rejected_recent} signal(s) rejected at entry in the last 24h "
                                f"(stale price / bad timeframe / duplicate) — check the individual alerts")
    ...
    for upd in result.get("result", []):
        ...
        try:
            ...
            if text.startswith("/start"):
                ...
                # 🔴 طبق درخواست صریح: هر سه پاسخِ این دستور دکمه‌ی «⬅️ Back to Menu»
                # می‌گیرن - قبلاً هیچ‌کدوم نداشتن.
                back_kb = [[{"text": "⬅️ Back to Menu", "callback_data": "menu:home"}]]
                parts = text.split(maxsplit=1)
                if len(parts) != 2:
                    send_message(chat_id, "Usage: /code YOURCODE", keyboard=back_kb)
                else:
                    ...
                    if d:
                        ...
                        send_message(chat_id, "That code is invalid, expired, or fully used.", keyboard=back_kb)
                ...
                if len(parts) != 3:
                    send_message(chat_id, "Usage: /setrisk BALANCE RISK_PERCENT — e.g. /setrisk 1000 2\n"
                                           ...
                                           "/pnl will automatically use these numbers.", keyboard=back_kb)
                else:
                    try:
                        ...
                        send_message(chat_id, "Both values must be numbers, e.g. /setrisk 1000 2", keyboard=back_kb)
                ...
                if active:
                    ...
                    send_message(chat_id, "You don't have an active membership.", keyboard=back_kb)
        except Exception as e:
            ...
            # 🔴 رفعِ باگِ واقعیِ «یک پستِ کانال با یک باگِ کاملاً پیش‌بینی‌نشده می‌تونه برای همیشه
            # غیرقابل‌بازیابی بشه»: چون state["last_update_id"] بالاتر (خط ۱۳۳۴) *قبل* از رسیدن
            # به اینجا قبلاً ست شده، این آپدیت دیگه هیچ‌وقت از تلگرام دوباره fetch نمی‌شه - یعنی
            # اگه خودِ متنِ پیام (که شاملِ هشتگِ شناسه‌ی سیگنال و مقدارِ سود/زیانه) همین‌جا در
            # هشدار نباشه، دیگه هیچ‌جا در دسترس نیست تا ادمین بتونه دستی جبرانش کنه - سیگنال
            # واقعاً برای همیشه گم می‌شه. این مورد فقط برای message/callback عادی بی‌خطره (کاربر
            # می‌تونه پیامش رو دوباره بفرسته)، ولی برای channel_post/edited_channel_post (پستِ
            # واقعیِ کانال - چیزی که هیچ‌کس نمی‌تونه "دوباره بفرسته") حیاتیه. الان اگه آپدیتِ
            # کرش‌کرده از نوع پستِ کانال باشه، متنِ کاملِ خامش مستقیم در هشدار می‌آد تا ادمین با
            # /admin_manual_signal یا /admin_close_trade بتونه دستی و فوری جبرانش کنه.
            raw_post = upd.get("channel_post") or upd.get("edited_channel_post")
            if raw_post is not None:
                raw_text = raw_post.get("text") or raw_post.get("caption") or "(no text/caption on this post)"
                # 🔴 رفعِ باگِ بحرانی: raw_text (متنِ خامِ کانال)، e (پیامِ خطا)، و traceback
                # همه می‌تونن هر کاراکتری داشته باشن - از جمله '<' (مثلاً traceback پایتون
                # تقریباً همیشه "<module>" داره). چون این پیام با parse_mode=HTML فرستاده
                # می‌شه، بدونِ escape، تلگرام کل پیام رو با 400 Bad Request رد می‌کرد - یعنی
                # همین هشدارِ حیاتی (که دقیقاً برای نجاتِ یک سیگنالِ گم‌شده طراحی شده بود)
                # هیچ‌وقت به ادمین نمی‌رسید.
                alert_admins_text(
                    f"⚠️ Bot crashed while processing a CHANNEL POST (update id={upd.get('update_id')}) — "
                    f"this post will NOT be retried automatically, so its raw text is included below in full "
                    f"so you can recover it manually (e.g. with /admin_manual_signal or /admin_close_trade):\n\n"
                    f"<code>{html.escape(raw_text)}</code>\n\n"
                    f"Error: {html.escape(str(e))}\n\n<code>{html.escape(traceback.format_exc()[-800:])}</code>")
            else:
                alert_admins_text(
                    f"⚠️ Bot failed to process update id={upd.get('update_id')}: {html.escape(str(e))}\n\n"
                    f"<code>{html.escape(traceback.format_exc()[-1200:])}</code>")
            try:
                _fallback_chat_id = (
...
def handle_photo_submission(msg, chat_id, user_id, photo_submissions):
    ...
    # 🔴 رفعِ باگ: قبلاً submission_id فقط "{user_id}-{now_ts()}" بود (دقتِ ثانیه‌ای). اگر همان
    # کاربر دو عکس را در یک ثانیه می‌فرستاد (مثلاً انتخاب چند عکس با هم به‌صورت آلبوم - در
    # تلگرام معمول است)، هر دو submission دقیقاً همان id را می‌گرفتند و دکمه‌ی Approve/Reject
    # روی پیامِ دومی همیشه رکورد اولی را تغییر می‌داد (next() همیشه اولین match را برمی‌گرداند) -
    # یعنی عکس دوم برای همیشه pending می‌ماند و هیچ‌وقت پاداش نمی‌گیرد. چون photo_submissions
    # هرگز آیتمی حذف نمی‌کند (فقط append)، طول فعلی لیست یک پسوند تضمین‌شده‌ی یکتا و کوتاه است.
    submission_id = f"{user_id}-{now_ts()}-{len(photo_submissions)}"
...
def handle_callback(cb, pending, subscribers, discounts, applied, used_tx, referrals, maintenance, photo_submissions):
    ...
    if data == "menu:results":
        # 🔴 رفعِ همون شکافِ «نتایج انگار اصلاً وجود نداشته‌اند»: مسیر تایپیِ /results
        # از قبل _ensure_fresh_data() را قبل از خواندن trade_history.json صدا می‌زد، ولی این
        # مسیر (کلیک روی دکمه‌ی منو - که در عمل محتمل‌ترین راهیه که کاربرها استفاده می‌کنن)
        # این pull را نداشت، یعنی همیشه با هر چی که آخرین‌بار چرخه‌ی دوره‌ای ۴۵-۱۲۰ثانیه‌ای
        # به‌روزش کرده بود جواب می‌داد - تا دقایق (یا با یک تعارض گذرا، بیشتر) عقب‌تر از واقعیت.
        _ensure_fresh_data()
        rows = [[{"text": "⬅️ Back", "callback_data": "menu:home"}]]
    ...
    if data.startswith("hist:"):
        _ensure_fresh_data()  # همون رفعِ شکاف - نگاه کنید کامنتِ menu:results بالاتر
    ...
    if data == "pnl:mine":
        ...
        # 🔴 رفعِ همون شکافِ «نتایج انگار اصلاً وجود نداشته‌اند» - format_pnl_message هم از
        # همون trade_history.json مشترک می‌خونه (نگاه کنید کامنتِ menu:results بالاتر).
        _ensure_fresh_data()
        profile = _load_risk_profile(user_id)
    ...
    # 🔴 اضافه شد: دکمه‌های /admin_list و /admin_user - طبق درخواستِ صریح («صفر تا صد راحت
    # باشم، همه‌چیز دکمه داشته باشه») - هر ۵ حالت دقیقاً از همون توابعِ مشترکی استفاده می‌کنن
    # که خودِ دستورِ متنی هم استفاده می‌کنه (_admin_extend_user/_admin_reduce_user/
    # _admin_remove_user/_format_admin_user_message/_format_admin_list_page) - یعنی این دو
    # مسیر (تایپ‌کردن یا کلیک‌کردن) هیچ‌وقت از هم drift نمی‌کنن.
    if data.startswith("alist:"):
        if user_id not in ADMIN_USER_IDS:
            return
        page = int(data.split(":", 1)[1])
        text, kb = _format_admin_list_page(subscribers, page)
        edit_message(chat_id, message_id, text, keyboard=kb)
        return

    if data.startswith("auser_view:"):
        if user_id not in ADMIN_USER_IDS:
            return
        target_id = int(data.split(":", 1)[1])
        text = _format_admin_user_message(target_id, subscribers, referrals, applied, pending)
        edit_message(chat_id, message_id, text, keyboard=_admin_user_keyboard(target_id))
        return

    if data.startswith("auser_add:"):
        if user_id not in ADMIN_USER_IDS:
            return
        _, target_id_s, days_s = data.split(":")
        target_id, days = int(target_id_s), int(days_s)
        result = _admin_extend_user(subscribers, target_id, days)
        text = _format_admin_user_message(target_id, subscribers, referrals, applied, pending)
        edit_message(chat_id, message_id, f"✅ {result}\n\n{text}", keyboard=_admin_user_keyboard(target_id))
        return

    if data.startswith("auser_reduce:"):
        if user_id not in ADMIN_USER_IDS:
            return
        _, target_id_s, days_s = data.split(":")
        target_id, days = int(target_id_s), int(days_s)
        result = _admin_reduce_user(subscribers, target_id, days)
        text = _format_admin_user_message(target_id, subscribers, referrals, applied, pending)
        edit_message(chat_id, message_id, f"✅ {result}\n\n{text}", keyboard=_admin_user_keyboard(target_id))
        return
    ...
    if data.startswith("auser_remove:"):
        if user_id not in ADMIN_USER_IDS:
            return
        target_id = int(data.split(":", 1)[1])
        result = _admin_remove_user(subscribers, target_id)
        edit_message(chat_id, message_id, f"✅ {result}")
        return
    ...
    # 🔴 اضافه شد: دکمه‌های زیر هشدار «Altcoin relay signal ... was NOT tracked» - وقتی یک
    # سیگنالِ رله‌شده فقط به‌خاطر تایم‌فریمِ نامعتبر/ناشناخته رد شده بود، به‌جای اینکه ادمین
    # کل دستور /admin_manual_signal رو با symbol/side/entry/sl دستی بازنویسی کنه، همون
    # جزییات (که در _queue_pending_manual_fix ذخیره شدن) رو با تایم‌فریمِ انتخاب‌شده دوباره
    # صف می‌کنه.
    if data.startswith("mfix:"):
        if user_id not in ADMIN_USER_IDS:
            return
        _, fix_id, tf = data.split(":", 2)
        fixes = _load("pending_manual_fixes.json", [])
        fix = next((f for f in fixes if f.get("id") == fix_id), None)
        if not fix:
            edit_message(chat_id, message_id, cb["message"].get("text", "") +
                         "\n\n❌ This fix request expired or was already used.")
            return
        ok, note = queue_manual_signal(fix["symbol"], fix["side"], str(fix["entry"]), str(fix["sl"]),
                                        forwarded_tracking=True, tf_label=tf, silent=True,
                                        source_signal_id=fix.get("source_signal_id"),
                                        signal_message_id=fix.get("signal_message_id"))
        result_line = f"\n\n✅ Queued with timeframe {tf} — will start tracking within a few seconds." if ok \
            else f"\n\n❌ Still failed with timeframe {tf}: {note}"
        edit_message(chat_id, message_id, cb["message"].get("text", "") + result_line)
        return
...
ADMIN_HELP = (
    ...
    "of the underlying cause — the result you give is counted in /results and the daily report immediately, "
    "and a channel close message goes out just like any other close. Also works on an already-closed signal "
    "(give an explicit final_r) to correct its recorded result — no duplicate channel message for a correction.\n\n"
    "<u>Pricing</u>\n"
...
# ================== اقدامِ مشترکِ ادمین روی یک کاربر (متن دستی + دکمه، هر دو از همینا استفاده می‌کنن) ==================
#
# 🔴 طبق درخواستِ صریح («صفر تا صد راحت باشم، همه‌چیز دکمه داشته باشه»): این سه تابع دقیقاً
# همون منطقی رو دارن که قبلاً فقط داخل بدنه‌ی /admin_add و /admin_reduce و /admin_remove
# تکرار شده بود - حالا یک‌بار اینجا نوشته شدن و هم دستورِ متنی هم دکمه‌ی callback از همین یکی
# استفاده می‌کنن؛ یعنی هیچ‌وقت این دو تا از هم جدا (drift) نمی‌شن.
def _admin_extend_user(subscribers, target_id: int, days: int) -> str:
    ...
    return f"Done. User {target_id} active until {fmt_date(new_expiry)}."


def _admin_reduce_user(subscribers, target_id: int, days: int) -> str:
    sub = _find_sub(subscribers, target_id)
    if not sub:
        return "That user has no active subscription."
    ...
    if sub["expiry_ts"] <= now_ts():
        ...
        return f"User {target_id}'s subscription reached zero and was removed."
    send_message(target_id, f"Your subscription was adjusted by an admin.\nNew expiry: {fmt_date(sub['expiry_ts'])}")
    return f"Done. User {target_id} now active until {fmt_date(sub['expiry_ts'])}."


def _admin_remove_user(subscribers, target_id: int) -> str:
    sub = _find_sub(subscribers, target_id)
    remove_member(target_id)
    if sub:
        subscribers.remove(sub)
    send_message(target_id, "Your membership has been removed by an admin.")
    return f"User {target_id} removed."


def _admin_user_keyboard(target_id: int):
    return [
        [{"text": "+7 days", "callback_data": f"auser_add:{target_id}:7"},
         {"text": "+30 days", "callback_data": f"auser_add:{target_id}:30"}],
        [{"text": "−7 days", "callback_data": f"auser_reduce:{target_id}:7"},
         {"text": "🗑 Remove", "callback_data": f"auser_remove:{target_id}"}],
        [{"text": "🔄 Refresh", "callback_data": f"auser_view:{target_id}"}],
    ]


def _format_admin_user_message(target_id: int, subscribers, referrals, applied, pending) -> str:
    sub = _find_sub(subscribers, target_id)
...
def _format_admin_list_page(subscribers, page: int):
    """متن + کیبورد یک صفحه از /admin_list - هم دستورِ متنی هم دکمه‌ی ⬅️/➡️ از همینا استفاده
    می‌کنن (دقیقاً همون الگوی history_keyboard/format_history_page)."""
    page_size = 50
    ...
    footer = "\n\nTap a page button below, or use /admin_user &lt;user_id&gt; to look up one person in detail."
    text = header + "\n".join(lines) + footer
    nav = []
    if page > 0:
        nav.append({"text": "⬅️ Prev", "callback_data": f"alist:{page-1}"})
    if page + 1 < total_pages:
        nav.append({"text": "Next ➡️", "callback_data": f"alist:{page+1}"})
...
def queue_manual_signal(symbol, side, entry_s, sl_s, forwarded_tracking=False, tf_label=None, silent=False,
                         source_signal_id=None, signal_message_id=None):
    """
    ...
    🔴 اضافه شد: signal_message_id - برای مسیرِ silent=True (رله‌ی خودکار آلتکوین)، پیامِ ورودِ
    محلی هیچ‌وقت پست نمی‌شه (همون‌طور که silent بالاتر توضیح می‌ده) - پس trade["signal_message_id"]
    در candle_engine.py هیچ‌وقت از _send_entry ست نمی‌شد، و در نتیجه اگه ادمین بعداً همین
    معامله رو با /admin_close_trade ببنده، پیامِ بسته‌شدن (که طبق درخواستِ صریحِ «هر پیام مربوط
    به سیگنالِ اولیه حتماً ریپلای شود» باید ریپلای کنه) هیچ چیزی برای ریپلای‌کردن نداشت. اینجا
    handle_altcoin_relay_post بلافاصله بعد از copyMessage، message_id همون کپیِ مقصد (در
    PRIVATE_CHANNEL_ID) رو پاس می‌ده - candle_engine.py مستقیماً همین رو روی
    trade["signal_message_id"] می‌ذاره (بدون نیاز به پستِ محلیِ جدید)، پس هر پیامِ بعدی
    (target/stop/admin-close/...) که از سمتِ خودِ candle_engine.py برای این معامله بره، درست
    ریپلای می‌کنه.
    """
    ...
    # 🔴 رفعِ باگِ واقعیِ و جدیِ «بعضی سیگنال‌های رله‌شده اصلاً در نتایج حساب نمی‌شدند» - دقیقاً
    # همون کلاسِ باگی که قبلاً برای شناسه‌ی عکس‌های سود پیدا و رفع شد، این‌بار در جایی به‌مراتب
    # حساس‌تر: signal_id قبلاً همیشه f"m-{now_ts()}" بود (دقتِ فقط تا سطح ثانیه). اگه دو
    # سیگنالِ *متفاوت* (مثلاً دو نماد مختلف، یا حتی دو سیگنالِ SOL جدا) در همون یک ثانیه صف
    # می‌شدن (کاملاً محتمل وقتی bot.py چند سیگنال را در یک چرخه‌ی اسکن پشتِ‌سرهم پست می‌کنه)،
    # هر دو دقیقاً همین id را می‌گرفتن. چون candle_engine.py از همین id برای ساختنِ کلیدِ
    # ردیابیِ داخلیِ معامله (state_key = f"MANUAL|{id}") استفاده می‌کنه، این برخورد باعث می‌شد
    # معامله‌ی دومی که پردازش می‌شه، جای معامله‌ی اولی را در حافظه بگیرد - معامله‌ی اول برای
    # همیشه «یتیم» می‌شد: نه دیگه کسی می‌تونست پیدایش کنه، نه نتیجه‌اش هیچ‌وقت در trade_history/
    # /results حساب می‌شد - دقیقاً همون الگویی که با مقایسه‌ی گزارشِ روزانه‌ی دو کانال دیده شد
    # (بعضی سیگنال‌ها کاملاً از آمار VIP غایب بودند). رفع: وقتی source_signal_id موجوده (که
    # برای هر سیگنالِ رله‌شده‌ی واقعی همیشه هست - همون هشتگِ خودِ پیام)، مستقیم از همون به‌عنوانِ
    # پایه‌ی id استفاده می‌کنیم - چون خودش تضمین‌شده یکتاست (دقیقاً طبق پیشنهادِ خودتان: از
    # شناسه‌ی منحصربه‌فردِ هر سیگنال کمک گرفته شود). برای موردِ نادرِ سیگنالِ دستیِ بدون هشتگِ
    # منبع (تایپِ مستقیمِ ادمین)، طولِ فعلیِ لیست (که هرگز آیتم حذف نمی‌کنه، پس تضمین‌شده یکتاست)
    # به‌عنوان پسوندِ ضدِتصادم اضافه می‌شه - دقیقاً همون الگویی که برای photo_submissions استفاده شد.
    if source_signal_id:
        signal_id = f"src-{source_signal_id}"
    else:
        signal_id = f"m-{now_ts()}-{len(signals)}"
    signals.append({
...
def handle_channel_signal_post(post):
    ...
    #
    # 🔴 رفعِ یک باگِ سیستمیِ واقعی (پیدا شد حین بررسیِ اینکه چرا هشدارهای «Forwarded result
    # never matched a trade» بی‌دلیل زیاد بودن): قبلاً این مسیر هیچ شرطی نداشت - یعنی روی
    # *هر* channel_post ای که فرمتِ پیامِ نتیجه رو داشت اجرا می‌شد، حتی پیام‌های خودِ
    # candle_engine.py که مستقیم (بدون فوروارد) پست می‌شن! نتیجه: هر بارِ خودکاری/زنده که یک
    # معامله می‌بست (استاپ/تارگت/...)، همون پیامِ خودِ ربات دوباره اینجا هم دیده و دوباره صف
    # می‌شد (queue_forwarded_result) - و چون اون معامله از قبل (توسط رهگیریِ زنده‌ی خودش) با
    # closed=True بسته شده بود، این رویدادِ تکراری هیچ‌وقت نمی‌تونست تطبیق پیدا کنه (تطبیق
    # نیازمند «not t.get('closed')»ه) - و بعد از FORWARDED_RESULT_MAX_ATTEMPTS تلاش، یک
    # هشدارِ کاملاً کاذبِ «unmatched» به ادمین می‌رفت. دقیقاً همین اتفاق برای کپیِ
    # copyMessage-شده‌ی نتایجِ رله‌ی آلتکوین هم می‌افتاد: handle_altcoin_relay_post خودش
    # مستقیماً پیامِ مبدأ رو پردازش/صف می‌کنه، ولی کپیِ همون پیام که با copyMessage توی همین
    # کانال (PRIVATE_CHANNEL_ID) دوباره ظاهر می‌شه، بعداً از همین‌جا (مسیر ۳) *دوباره* صف
    # می‌شد - یک هشدارِ کاذبِ دومی برای همون یک بسته‌شدن. یعنی عملاً هر بسته‌شدنی در کل سیستم
    # (چه خودکار چه رله‌ی آلتکوین) یک هشدارِ کاذبِ اضافه تولید می‌کرد.
    #
    # رفع: دقیقاً همون شرطی که مسیر ۲ از قبل برای تشخیصِ رهگیریِ فوروارد استفاده می‌کنه
    # (is_forwarded_message) اینجا هم اعمال شد - این مسیر فقط برای پیام‌هایی که تلگرام واقعاً
    # به‌عنوان فوروارد علامت زده (یعنی دقیقاً همون caseی که این pipeline اصلاً برایش طراحی شده:
    # یک ادمین دستی یک نتیجه از یک منبع خارجی فوروارد می‌کنه) اجرا می‌شه. پیام‌های خودِ
    # candle_engine.py هیچ‌وقت «فوروارد» نیستن (پستِ اصلی خودشونن) و copyMessage هم عمداً
    # متادیتای فوروارد رو منتقل نمی‌کنه (طبق مستندات API تلگرام) - پس این شرط هر دو کلاسِ
    # تکرارِ کاذب رو از ریشه حذف می‌کنه، بدون از دست دادن هیچ قابلیتِ واقعی.
    if is_forwarded_message(post):
...
def handle_altcoin_relay_post(post, relay_seen_ids, relay_msgid_map):
    ...
    if src_reply:
        ...
        if dest_reply_to is None:
            # 🔴 رفعِ باگِ واقعیِ «بعضی پیام‌های نتیجه در کانالِ VIP بدون نوارِ ریپلای/کوتیشن
            # می‌مونن»: نگاشتِ message_id مبدأ→مقصد بالا هر ۲۰۰۰ ورودیِ اخیر رو نگه می‌داره
            # (save_all پایین‌تر) - برای معامله‌ای که طولانی باز بمونه یا در یک بازه‌ی
            # پرترافیک بسته بشه، ممکنه ورودیِ پیامِ سیگنالِ اصلی قبل از رسیدنِ نتیجه‌اش evict
            # بشه، و آن‌وقت نتیجه بدون ریپلای/کوتیشن پست می‌شه - نه یک باگِ محاسباتی (هنوز
            # درست بسته و حساب می‌شه)، فقط یک ناهماهنگیِ بصری. چون هر پیامِ نتیجه‌ی
            # candle_engine.py/bot.py همیشه همون هشتگِ شناسه‌ی سیگنالِ ورودی رو هم داره، یک
            # راهِ دومِ کاملاً مستقل از حجمِ ترافیک برای پیداکردنِ همون پیامِ مقصد داریم: مستقیم
            # با خودِ شناسه (نه با زنجیره‌ی message_id). این کلید هیچ‌وقت evict نمی‌شه چون
            # جداگانه (با پیشوندِ "sid:") توسط سیگنال‌های ورودی ثبت می‌شه، نه هر پیامِ رله‌شده.
            src_text = src_reply.get("text") or src_reply.get("caption") or ""
            src_sid = _extract_source_signal_id(src_text)
            if src_sid:
                dest_reply_to = relay_msgid_map.get(f"sid:{src_sid}")
    ...
    # 🧹 تمیزکاری: این مقدار در دو جای پایین لازم است (ثبتِ کلیدِ sid: و شاخه‌ی اصلیِ پردازشِ
    # سیگنالِ ورودی) - یک‌بار محاسبه می‌شود تا starts_with_signal_emoji(text) دوبار صدا زده نشود.
    is_entry_signal = starts_with_signal_emoji(text)
    if dest_message_id:
        ...
        # 🔴 رفعِ باگِ واقعیِ «بعضی نتایج بدون ریپلای/کوتیشن می‌مونن» (توضیحِ کامل بالاتر، جایی
        # که dest_reply_to محاسبه می‌شه): علاوه بر کلیدِ message_id (که با ترافیکِ زیاد evict
        # می‌شه)، اگه این پیام خودش سیگنالِ *ورودیِ* اصلی (نه یک نتیجه) باشه، با کلیدِ
        # "sid:<id>" هم ثبتش می‌کنیم - این کلید کاملاً مستقل از حجمِ کلیِ پیام‌های رله‌شده‌ست
        # (فقط به تعداد سیگنال‌های واقعی رشد می‌کنه، نه هر پیام)، پس عمرش خیلی بیشتر از حدِ
        # evict معمولیه. عمداً فقط برای پیامِ ورودی و نه هر پیامی که این هشتگ رو داره: اگه
        # بدون این قید بود، اولین پیامِ *نتیجه* (مثلاً «Target 1 HIT») که همون هشتگ رو داره
        # خودش می‌تونست این کلید رو با dest_message_id خودش (نه پیامِ ورودی) بازنویسی کنه - و
        # آن‌وقت نتیجه‌ی بعدی به‌جای ریپلای به سیگنالِ اصلی، به یک پیامِ نتیجه‌ی دیگه ریپلای می‌کرد.
        if is_entry_signal:
            entry_sid = _extract_source_signal_id(text)
            if entry_sid:
                relay_msgid_map[f"sid:{entry_sid}"] = dest_message_id

    if is_entry_signal:
        parsed = parse_forwarded_signal_text(text)
        if not parsed:
            # 🔴 رفعِ یک «گم‌شدنِ کاملاً بی‌صدا»ی واقعی: قبلاً وقتی متنِ منبع با ایموجی سیگنال
            # شروع می‌شد ولی parse_forwarded_signal_text نمی‌تونست هدر/Entry/Stop رو دقیق
            # استخراج کنه (مثلاً یک تغییرِ جزئیِ فرمت در دیپلوی آلتکوین، یک کاراکترِ نامرئی،
            # یا هر فرمتِ غیرمنتظره‌ی دیگه)، هیچ اتفاقی نمی‌افتاد - نه صف می‌شد، نه هشداری
            # می‌رفت. نتیجه دقیقاً همون چیزی بود که دیده شد: سیگنال با چارت/متنش کاملاً درست
            # در کانال دیده می‌شه (copyMessage بالاتر همیشه موفق بود)، ولی برای محاسبه/گزارش
            # عملاً هیچ‌وقت وجود نداشت - و چون هیچ هشداری هم نمی‌رفت، حتی خودِ ادمین هم تا
            # وقتی کسی دستی /signal بزنه متوجه نمی‌شد. الان هر پیامی که با ایموجیِ سیگنال
            # شروع بشه ولی قابل‌پارس نباشه، بلافاصله با متنِ خامش به ادمین‌ها گزارش می‌شه - تا
            # بشه دستی با /admin_manual_signal یا #signal دوباره واردش کرد.
            alert_admins_text(
                f"⚠️ Altcoin relay: a message starting with the signal emoji could not be parsed "
                f"(no clear LONG/SHORT — symbol header, or missing Entry:/Stop: lines) — it WAS "
                f"mirrored into the channel (so it's visible there), but it will NOT be tracked/"
                f"counted anywhere unless entered manually.\n\n"
                f"Raw text:\n<code>{html.escape(text[:600])}</code>\n\n"
                f"To track it manually: <code>#signal SYMBOL/USD BUY_or_SELL TF ENTRY STOP</code> "
                f"(reply with that in this channel, or use /admin_manual_signal in this chat).")
            return
        symbol, side, entry, sl, tf_label, source_signal_id = parsed
        # 🔴 رفعِ آخرین شکافِ سکوت در این تابع: تا اینجا فقط دو مسیر شکست (پارس‌نشدن، رد‌شدنِ
        # queue_manual_signal) به ادمین اطلاع می‌دادن؛ این مسیر سوم (تشخیصِ تکراری) کاملاً
        # بی‌صدا بود - اگه is_duplicate_signal اشتباهاً True برمی‌گردوند (false positive)، این
        # سیگنال مثل مسیرهای دیگه در کانال دیده می‌شد ولی هیچ‌جا رهگیری نمی‌شد، بدون هیچ اثری
        # حتی برای خودِ ادمین. الان این مسیر هم مثل بقیه هشدار می‌ده.
        if is_duplicate_signal(symbol, side, entry, sl):
            alert_admins_text(
                f"⚠️ Altcoin relay signal {symbol} {side} entry={entry} sl={sl} was mirrored into "
                f"the channel but treated as a DUPLICATE of a recent signal (same symbol/side/entry/"
                f"stop within the last 6h) — so it was NOT tracked. If this was actually a genuinely "
                f"new signal, use /admin_manual_signal to enter it manually with these exact values.")
        else:
            ...
            ok, note = queue_manual_signal(symbol, side, str(entry), str(sl),
                                            ...
                                            signal_message_id=dest_message_id)
            if not ok:
                ...
                # (معمولاً چون تایم‌فریمِ منبع نامعتبر/ناشناخته بود)، دیگه هیچ
                # رهگیری/محاسبه‌ای براش انجام نمی‌شه - قبلاً این حالت کاملاً بی‌صدا از
                # دست می‌رفت (نه پیامی، نه هشداری).
                #
                # 🔴 طبق درخواست صریح («به‌جای تایپ‌کردن دستورات، دکمه‌ی اقدام لازم»):
                # چون علتِ رایجِ این خطا خودِ تایم‌فریمه (که نمی‌دونیم دقیقاً کدومه)، به‌جای
                # فقط توضیح متنی، ۵ دکمه - یکی برای هر تایم‌فریمِ معتبر - نشون داده می‌شه؛
                # با یک تپ، همون سیگنال (symbol/side/entry/sl/source_signal_id که از قبل
                # پارس شدن) با تایم‌فریمِ انتخابی صف می‌شه - نیازی به تایپِ دستیِ
                # /admin_manual_signal با همه‌ی این پارامترها نیست.
                fix_id = _queue_pending_manual_fix(symbol, side, entry, sl, source_signal_id, note,
                                                    signal_message_id=dest_message_id)
                tf_buttons = [{"text": tf, "callback_data": f"mfix:{fix_id}:{tf}"}
                              for tf in sorted(VALID_TIMEFRAME_LABELS)]
                tf_rows = [tf_buttons[i:i + 3] for i in range(0, len(tf_buttons), 3)]
                alert_admins_text(
                    ...
                    f"Tap the timeframe this signal was actually posted on to track it now:",
                    keyboard=tf_rows)
        return
...
def handle_admin_command(chat_id, text, subscribers, discounts, maintenance, photo_submissions,
    ...
    if cmd in ("/admin_list", "/admin"):
        ...
        # در هر صفحه) تا در هر مقیاسی همیشه قابل‌استفاده بمونه.
        if not subscribers:
        ...
        if len(parts) > 1:
            try:
                ...
                pass
        # 🔴 طبق درخواستِ صریح («صفر تا صد راحت باشم، بدون تایپ‌کردن»): دیگه لازم نیست ادمین
        # برای صفحه‌ی بعدی /admin_list N رو دستی تایپ کنه - دکمه‌ی ⬅️/➡️ همون کاری رو می‌کنه
        # (دقیقاً همون الگوی history_keyboard).
        text, kb = _format_admin_list_page(subscribers, page)
    ...
    if cmd == "/admin_user":
        ...
        try:
            ...
            return
        # 🔴 طبق درخواستِ صریح: دیگه لازم نیست ادمین برای تمدید/کاهش/حذف این کاربر دستور
        # جداگانه‌ای تایپ کنه - همون‌جا زیرِ همین پیام، دکمه‌ی هر اقدام هست.
        text = _format_admin_user_message(target_id, subscribers, referrals, applied, pending)
        send_message(chat_id, text, keyboard=_admin_user_keyboard(target_id))
        return
    ...
    if cmd in ("/admin_add", "/admin_extend"):
        ...
        try:
            ...
            return
        send_message(chat_id, _admin_extend_user(subscribers, target_id, days))
        return
    ...
    if cmd == "/admin_reduce":
        ...
        try:
            ...
            send_message(chat_id, "user_id and days must be numbers.")
            return
        send_message(chat_id, _admin_reduce_user(subscribers, target_id, days))
        return
    ...
    if cmd == "/admin_remove":
        ...
        try:
            ...
            send_message(chat_id, "user_id must be a number.")
            return
        send_message(chat_id, _admin_remove_user(subscribers, target_id))
        return
    ...
    if cmd == "/admin_close_trade":
        ...
        # هشتگ پیام کانال) است؛ کلید قدیمی‌تر (مثلاً «BTC/USD|4h» یا «MANUAL|m-1787774585»)
        ...
        #
        # 🔴 طبق درخواست صریح: این دستور الان دقیقاً همون یک ابزار برای هر دوی این کاراست -
        # (۱) بستنِ یک سیگنالِ هنوز-باز (نتیجه بلافاصله در /results و گزارش روزانه حساب
        # می‌شه، و یک پیام «⚠️ TRADE CLOSED» عیناً هم‌شکل با بقیه‌ی پیام‌های بسته‌شدن به کانال
        # ارسال می‌شه)، و (۲) ویرایشِ نتیجه‌ی یک سیگنالِ از قبل بسته‌شده - دقیقاً همین دستور،
        # فقط با یک final_r صریح، روی هر سیگنالِ بسته‌شده هم کار می‌کنه (بدون پیامِ تکراری به
        # کانال - چون خبرِ بسته‌شدنش یک‌بار قبلاً درست منتشر شده، فقط عددِ آماره اصلاح می‌شه).
        if len(parts) not in (2, 3):
            send_message(chat_id, "Usage: /admin_close_trade <signal_id or key> [final_r]\n"
                                   ...
                                   "This also works on an already-closed signal to correct its recorded "
                                   "result — just give an explicit final_r; no duplicate channel message "
                                   "is sent for a correction, only for a genuine first close.")
            return
        key = parts[1]
        # 🔴 رفعِ یک باگِ واقعی: تا الان این دستور ورودی رو دقیقاً همون‌طور که تایپ می‌شد
        # (بدون نرمال‌سازی) پاس می‌داد. اما راهنمای خودِ همین دستور (و متنِ /signal) صریحاً
        # می‌گه «هشتگِ زیرِ پیامِ کانال رو کپی کن» - و اون هشتگ همیشه با آندرلاین است
        # («#ALT_000072»)، نه خط‌تیره («ALT-000072»)! چون candle_engine.py فقط یک مقایسه‌ی
        # دقیقِ رشته‌ای انجام می‌ده (t["signal_id"] == key)، "ALT_000072" هرگز با
        # "ALT-000072" ذخیره‌شده تطبیق پیدا نمی‌کرد - حتی برای یک سیگنالِ کاملاً معتبر و
        # پیداشدنی. الان دقیقاً همون نرمال‌سازیِ _normalize_signal_id که /signal از قبل
        # استفاده می‌کنه، اینجا هم اعمال می‌شه - فقط وقتی کلید شبیهِ یک signal_id باشه (نه
        # وقتی "/" یا "|" داره، که یعنی کلیدِ قدیمی‌ترِ state_key مثل «BTC/USD|4h» یا
        # «MANUAL|m-1787774585» است - اون‌ها دست‌نخورده می‌مونن، چون حساسِ بزرگ/کوچکیِ حروف و
        # فرمتِ اصلی‌شون برای تطبیق در candle_engine.py حیاتیه).
        if "/" not in key and "|" not in key:
            key = _normalize_signal_id(key) or key
        final_r = None
        if len(parts) == 3:
            try:
        ...
        send_message(chat_id, f"✅ Queued — candle_engine.py will close/correct '{key}' ({final_r_desc}) "
                               f"within a few seconds. You'll get an alert if the key wasn't found anywhere "
                               f"(open or history), or if it's already closed and you didn't give an explicit "
                               f"final_r to correct it with.")
        return
...
def alert_admins_text(text: str, keyboard=None) -> None:
    ...
    🔴 اضافه شد: پارامتر اختیاری keyboard - برای هشدارهایی که یک اقدامِ مشخص و محدود دارن
    (مثلاً «این سیگنال رو با کدوم تایم‌فریم صف کنم؟») به‌جای اینکه ادمین مجبور باشه یک
    دستور رو کامل دستی تایپ کنه، دکمه‌های آماده زیر پیام نشون داده می‌شه."""
    for admin_id in ADMIN_USER_IDS:
        try:
            send_message(admin_id, text, keyboard=keyboard)
        except Exception as e:
            print(f"[warn] failed to alert admin {admin_id}: {e}")


_GIT_ERROR_MESSAGES = {
    # 🔴 رفعِ باگِ بحرانی («۴۰۰ Bad Request» مکرر که در لاگِ واقعی دیده شد): msg اینجا متنِ
    # خامِ خروجیِ گیته - می‌تونه هر چیزی داشته باشه، از جمله دقیقاً چیزی مثل
    # "git add/rm <conflicted_files>" که یک تگِ HTML نامعتبر به‌نظر می‌رسه. چون این پیام با
    # parse_mode=HTML فرستاده می‌شه، بدونِ escape، خودِ هشدار همیشه رد می‌شد - یعنی دقیقاً
    # وقتی یک مشکلِ واقعی (مثل تعارضِ دائمیِ گیت) پیش می‌اومد، ادمین هیچ‌وقت خبردار نمی‌شد.
    "not_a_repo": lambda msg: f"⚠️ subscription_bot.py: {html.escape(msg)}",
    "add_failed": lambda msg: f"⚠️ subscription_bot.py: git add data/ failed: {html.escape(msg)}",
    "commit_failed": lambda msg: f"⚠️ subscription_bot.py: git commit failed: {html.escape(msg)}",
    "rebase_conflict": lambda msg: f"⚠️ subscription_bot.py: {html.escape(msg)}",
    "rebase_stuck_recovered": lambda msg: f"🛠️ subscription_bot.py: {html.escape(msg)}",
    "rebase_stuck_recovery_failed": lambda msg: f"🛑 subscription_bot.py: {html.escape(msg)}",
    "push_failed": lambda msg: (
        f"⚠️ subscription_bot.py: {html.escape(msg)} Local data changes will NOT be visible to "
...
def git_commit_and_push(final: bool = False):
    ...
    def on_error(kind: str, message: str) -> None:
        ...
        text = _GIT_ERROR_MESSAGES.get(kind, lambda m: f"⚠️ subscription_bot.py: {html.escape(m)}")(message)
        if final:
...
def save_all(state, pending, subscribers, discounts, applied, used_tx, referrals, maintenance, photo_submissions,
    ...
    # 🔴 رفعِ باگ: کلیدهای "sid:<شناسه>" (نگاه کنید handle_altcoin_relay_post - fallback
    # دومِ پیداکردنِ dest_reply_to وقتی خودِ زنجیره‌ی message_id قبلاً evict شده) را جداگانه
    # و با سقفِ خودشون نگه می‌داریم، نه قاطی با کلیدهای عددیِ معمولی. چون این کلیدها فقط به
    # ازای هر *سیگنالِ واقعی* اضافه می‌شن (نه هر پیامِ رله‌شده - که شاملِ همه‌ی نتایج هم می‌شه)،
    # تعدادشون خیلی کمتر از ۲۰۰۰ کلِ رشدِ ترافیکه؛ اگه قاطی هم بمونن، ممکنه دقیقاً همون
    # کلیدهایی که این fallback برای نجاتشون طراحی شده بود، زودتر از موعد evict بشن.
    sid_items = [(k, v) for k, v in relay_msgid_map.items() if k.startswith("sid:")]
    plain_items = [(k, v) for k, v in relay_msgid_map.items() if not k.startswith("sid:")]
    trimmed_map = dict(plain_items[-2000:])
    trimmed_map.update(dict(sid_items[-3000:]))
...
def main():
    ...
    while time.time() - start < LOOP_MAX_SECONDS:
        ...
        cycles += 1
        # 🔴 رفعِ یک ناهماهنگی واقعی نسبت به الگوی حفاظتیِ candle_engine.py::main(): اونجا
        # هر مرحله‌ی حلقه‌ی اصلی (شاملِ git_commit_and_push) جداگانه try/except با notify_admin
        # داره، پس یک استثنای پیش‌بینی‌نشده در هر مرحله فقط همون یک مرحله رو رد می‌کنه، نه کل
        # پروسه رو می‌کشه. اینجا (subscription_bot.py) فقط run_cycle این محافظت رو داشت؛
        # save_all/send_heartbeat کاملاً بدون try/except بودن - یعنی مثلاً یک OSError نادر حین
        # نوشتن یکی از این ۱۰ فایل JSON (دیسک پر، مشکل مجوز) کل حلقه‌ی اصلی رو با یک exception
        # کنترل‌نشده می‌کشت، بدون اینکه حتی یک پیام به ادمین برسه (چون هیچ notify/alert ای بین
        # راه صدا زده نمی‌شد) - دقیقاً برخلاف فلسفه‌ی صریحِ این کدبیس («هیچ خطایی نباید بی‌صدا
        # از بین بره»، نگاه کنید shared_git_sync.py/read_json_resilient). git_commit_and_push
        # خودش داخلی هیچ‌وقت raise نمی‌کنه (sync_data_dir تمام خطاهای git رو با on_error می‌گیره
        # نه exception)، پس try/except دورش صرفاً برای تقارن/دفاعِ اضافی‌ست - نه یک شکافِ
        # جداگانه.
        if time.time() - last_commit >= GIT_COMMIT_EVERY_SECONDS:
            try:
                ...
                print(f"[error] periodic save/commit/heartbeat failed: {e}")
                try:
                    alert_admins_text(f"⚠️ subscription_bot.py: periodic save/commit/heartbeat failed unexpectedly: {html.escape(str(e))}")
                except Exception:
                    pass
    ...
    try:
        ...
        print(f"[error] final save/commit failed: {e}")
        try:
            alert_admins_text(f"⚠️ subscription_bot.py: final save/commit (end of Job) failed unexpectedly: {html.escape(str(e))} — "
                               f"anything not saved/pushed is likely lost until manually checked.")
        except Exception:
            pass
...
if __name__ == "__main__":
    # 🔴 رفعِ همون ناهماهنگی نسبت به candle_engine.py در سطح بالاتر: اونجا فراخوانیِ main()
    # از قبل با یک try/except + notify_admin("main (fatal)", e) + raise دوباره احاطه شده بود
    # (تا هم ادمین فوراً با پیام تلگرام باخبر بشه، هم GitHub Actions خودِ Job رو failed علامت
    # بزنه - نه اینکه فقط با کدِ خروجِ صفر و یک traceback خام در لاگ تموم بشه که کسی معمولاً
    # چکش نمی‌کنه). اینجا این محافظت اصلاً وجود نداشت.
    try:
        main()
    except Exception as e:
        print(f"[fatal] Unhandled exception in main(): {e}")
        try:
            alert_admins_text(f"🛑 subscription_bot.py crashed with an unhandled error and stopped: {html.escape(str(e))}\n"
                               f"The next scheduled run will start fresh from the last successfully "
                               f"saved/pushed state.")
        except Exception:
            pass
        raise
