...
def maybe_send_daily_report():
    ...
    برای اینکه این گزارش قطعاً «فقط یک‌بار» در روز فرستاده بشه، پرچمِ «امروز فرستاده شد» رو
    در یک فایل جدا (نه داخل state.json بزرگ که مدام توسط بخش‌های دیگه بازنویسی می‌شه) نگه
    می‌داریم، و قبل از تلاش برای ارسال پیام، همون لحظه ثبتش می‌کنیم (mark-then-send) - نه بعد
    از ارسال. این‌طوری حتی اگه یک دور اسکن هم‌زمان/فوری دیگه (یا اجرای بعدی که با تاخیر چند
    ثانیه‌ای start شده) دوباره به همین شرط برسه، پرچم رو از قبل می‌بینه و رد می‌شه."""
    ...
    # mark-then-send: همین الان ثبت می‌کنیم که امروز گزارش شد، قبل از اینکه واقعاً بفرستیم
...
def _force_close_trade(display, tf_key, tf_cfg, trade, candle, hist, symbol):
    ...
    exit_price = candle["c"]
    chart = build_chart_from_hist(hist, f"{_dt(display, tf_cfg['label'])} · Closed (new signal)", trade=trade)
...
def _send_exit(display, tf_key, tf_cfg, trade, event, hist, symbol, live_price=None):
    ...
    title = f"{_dt(display, tf_cfg['label'])} · {title_suffix}"
    # چارتِ لحظه‌ی برخورد به تارگت/استاپ باید واقعاً وضعیت زنده‌ی بازار همون لحظه باشه، نه
    # صرفاً یک نقطه‌ی «Live» روی چارتِ ذخیره‌شده‌ی قدیمی (که ممکنه مربوط به لحظه‌ی باز شدن
    # سیگنال، ساعت‌ها/روزها قبل باشه). یک دور کندل تازه می‌گیریم؛ اگه به هر دلیلی (قطعی
    # شبکه/سقف بودجه) نشد، به تاریخچه‌ی ذخیره‌شده به‌عنوان جایگزین برمی‌گردیم.
    fresh_hist = fetch_chart_hist(symbol)
    chart = build_chart_from_hist(fresh_hist or hist, title, trade=trade, live_price=live_price)
    if event["type"] == "stop":
