...
# دقیقاً طبق قبل کار می‌کنن. پیش‌فرض «1m» است (طبق تصمیم صریح کاربر)؛ برای برگردوندنش، فقط
# کافیه این secret رو حذف کنید یا خالی بذارید.
# 🔴 رفعِ باگِ واقعی (که این کامنت خودش قصدش رو داشت ولی کدِ زیرش تضمینش نمی‌کرد): وقتی
# candle_signals.yml این secret رو بدون قید و شرط پاس می‌ده، «خالی گذاشتنِ» secret باعث
# می‌شه os.environ.get یک رشته‌ی *موجود ولی خالی* برگردونه - نه غایب - یعنی پیش‌فرضِ دوم‌آرگومانیِ
# "1m" هیچ‌وقت اعمال نمی‌شد؛ به‌جاش "".split(",") -> [""] -> فیلتر می‌شد -> یک set خالی
# (یعنی هیچ تایم‌فریمی غیرفعال نمی‌شد - دقیقاً برعکسِ همون رفتار پیش‌فرضی که این کامنت قول
# داده). با الگوی or، خالی‌بودن و غایب‌بودن یکسان به همون پیش‌فرضِ مستندشده تبدیل می‌شن.
