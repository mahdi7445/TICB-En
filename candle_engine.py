...
def _id_line(trade: Dict[str, Any]) -> str:
    """خط شناسه‌ی یکتای سیگنال («🆔 TC-004821 #TC_004821») - در پیام ورود و هر پیام
    خروج/تارگت/استاپ نشون داده می‌شه، تا هرکسی که کانال رو دنبال می‌کنه (یا ادمین، برای
    /admin_close_trade) بتونه دقیقاً به همون یک سیگنالِ مشخص اشاره کنه - حتی وقتی چند
    سیگنالِ پی‌درپی روی یک نماد/تایم‌فریم جابه‌جا می‌شن. اگه به هر دلیلی (رکورد خیلی قدیمی،
    قبل از این فیچر) شناسه موجود نباشه، رشته‌ی خالی برمی‌گردونه - هیچ خط اضافه‌ای به پیام
    اضافه نمی‌شه."""
...
def process_admin_close_requests():
    ...
    for r in pending:
        ...
        if not t:
            ...
            # داده بشه»): قبلاً این هشدار فقط توضیح می‌داد key اشتباهه، بدون نشون‌دادن اینکه
            # پس دقیقاً کدوم signal_id ها الان واقعاً باز هستن - ادمین باید دستی می‌رفت
            # /admin_status یا کانال رو می‌گشت. حالا لیستِ همه‌ی معاملاتِ هنوز-بازِ فعلی
            # (با signal_id درست‌شون) + دستور آماده‌ی /admin_close_trade برای هرکدوم، دقیقاً
            # زیر همین پیام خطا میاد.
            open_now = [(kk, s.get("open_trade")) for kk, s in candle_states.items()
                        if (s or {}).get("open_trade") and not s["open_trade"].get("closed")]
            lines = [f"  • {ot.get('display', '?')} ({ot.get('side', '?')}, entry {ot.get('entry', '?')}) — "
                     f"{ot.get('signal_id', kk)}" for kk, ot in open_now[:15]]
            commands = [f"  <code>/admin_close_trade {ot.get('signal_id', kk)}</code>" for kk, ot in open_now[:15]]
            open_block = ("\n\nCurrently open trades (pick the right signal_id):\n" + "\n".join(lines) +
                          "\n\nReady-to-run commands (copy, optionally add a final_r, then send):\n" +
                          "\n".join(commands)) if open_now else "\n\nNo trades are open at all right now."
            not_found_text = (
                f"⚠️ <b>/admin_close_trade — signal not found</b>\n\n"
                f"Requested key: <code>{key}</code>\n\n"
                f"No matching OPEN trade was found (already closed, or the key/signal_id doesn't exist). "
                f"Double-check against the stale-trade alert message it came from — prefer the signal_id "
                f"(e.g. TC-004821 or ALT-004821) shown there over the older symbol|timeframe key when both "
                f"are available."
                f"{open_block}"
            )
            ...
            # باز/دستورات آماده (که دقیقاً همین ارزشِ این هشدار است) نصفه یا کامل حذف می‌شد.
            notify_admin(f"admin_close_not_found:{key}", Exception(not_found_text), full_text=not_found_text)
            continue
