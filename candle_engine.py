...
TF_CHECK_INTERVAL_SECONDS = {
    "1m": 12 * 60,   # طبق درخواست شما، تمرکز اصلی روی تایم‌فریم‌های پایین - تنگ‌تر از قبل (۱۵ دقیقه)
    "5m": 10 * 60,
    "15m": 15 * 60,
    "1h": 60 * 60,
    "4h": 4 * 60 * 60,
...
# جمع تخمینی: (120+144+96+24+6+1) × ۲ نماد ≈ ۷۸۲ درخواست/روز، زیر سقف ۸۰۰
# ⚠️ ۱۵m/1h/4h/1d همین الان هم دقیقاً به‌اندازه‌ی فاصله‌ی بسته‌شدن کندل خودشون چک می‌شن -
# یعنی از نظر بودجه‌ای دیگه جایی برای فشرده‌تر شدن ندارن (چک بیشتر از این باعث درخواست‌های
# تکراری روی یک کندل بسته‌نشده می‌شد، نه سیگنال زودتر). تنها جایی که واقعاً می‌شد فشرده‌تر
# کرد 1m بود؛ 5m همین الان (۱۰ دقیقه) هم نسبتاً تنگه و با توجه به سقف بودجه، فشرده‌تر کردنش
# همزمان با 1m جا نمی‌شد. با حذف عکس چارت از پیام‌های خروج (که هر بار یک درخواست اضافه
# مصرف می‌کرد)، بودجه‌ی آزادشده صرف تنگ‌تر کردن 1m شد. اگه بعد از دیپلوی، از داشبورد
# Twelve Data دیدید هنوز به سقف ۸۰۰ نزدیک نیستید، می‌شه 5m رو هم بیشتر فشرده کرد.
...
def format_daily_report_message(date_str: str, trades: List[Dict[str, Any]]) -> str:
    ...
    for h in trades:
        ...
        lines.append(f"{arrow} {symbol_part} — {h.get('close_reason', h.get('close_type','?'))} ({h['final_r']:+.2f}R)")
    return "\n".join(lines)
...
#
# مدیریت پوزیشن (به‌روزرسانی‌شده برای گرفتن بازدهی بیشتر از هر سیگنال):
#   Target 1 (R=1): بستن ۳۵٪ حجم پوزیشن (قبلاً ۵۰٪ — با بانک‌کردن کمتر در همون ابتدا،
#                   سهم بیشتری از پوزیشن برای تارگت‌های بالاتر باقی می‌مونه)
#   Target 2 (R=2): بستن ۳۵٪ از حجم باقی‌مانده + انتقال حد ضرر به نقطه ورود (ریسک‌فری)
#   Target 3 (R=3): بستن ۶۰٪ از حجم باقی‌مانده + شروع تریل‌کردن حد ضرر
#   Target 4 (R=4): بستن ۸۰٪ از حجم باقی‌مانده + اجازه دادن به ۲۰٪ باقی («Runner») که با
...
# سیگنال شما که دست‌نخورده مونده)، و جهتش با فلسفه‌ی خودِ استراتژی (فیلتر ADX≥20 - یعنی فقط
# در بازار روند‌دار سیگنال می‌ده) هم‌راستاست: چون قراره روند بگیریم، ماندگاری بیشتر روی
# پوزیشن منطقی‌تره. اما نمی‌تونم بدون بک‌تست واقعی روی داده‌ی تاریخی تضمین کنم که نتیجه‌ی
# نهایی بهتر می‌شه - این trade-off «سود بیشتر در روند قوی» در برابر «برگشت بیشتر در نوسان
# رنج» است. بعد از مدتی، نتایج واقعی /results رو با این تنظیم جدید مقایسه کنید.
...
# درصدهای بسته‌شدن در هر مرحله (هر کدوم درصدی از باقی‌مانده‌ی همون لحظه، نه از کل پوزیشن)
T1_CLOSE_PCT = 0.35              # در Target 1، از کل پوزیشن
T2_CLOSE_PCT_OF_REMAINDER = 0.35  # در Target 2، از باقی‌مانده
T3_CLOSE_PCT_OF_REMAINDER = 0.60  # در Target 3، از باقی‌مانده
T4_CLOSE_PCT_OF_REMAINDER = 0.80  # در Target 4، از باقی‌مانده (بقیه = Runner)

# وزن هر مرحله از حجم اصلی پوزیشن (مشتق‌شده از درصدهای بالا)
_W1 = T1_CLOSE_PCT
_rem_after_1 = 1 - _W1
_W2 = _rem_after_1 * T2_CLOSE_PCT_OF_REMAINDER
_rem_after_2 = _rem_after_1 - _W2
_W3 = _rem_after_2 * T3_CLOSE_PCT_OF_REMAINDER
_REMAINDER_AT_T4 = _rem_after_2 - _W3        # از حجم اصلی به تارگت ۴ می‌رسه
_RUNNER_KEEP_FRACTION = 1 - T4_CLOSE_PCT_OF_REMAINDER   # از همون باقی‌مانده به‌عنوان Runner باز می‌مونه
_W4 = _REMAINDER_AT_T4 * T4_CLOSE_PCT_OF_REMAINDER
...
TARGET_ACTION_LINE = {
    1: "\n\n✂️ Close 35% of your position here.",
    2: "\n\n✂️ Close 35% of your remaining position, and move your stop-loss to entry (risk-free).",
    3: "\n\n✂️ Close 60% of your remaining position, and start trailing your stop-loss.",
    4: "\n\n✂️ Close 80% of your remaining position. Let the final 20% run with your trailing "
...
def format_rr_exit_message(display: str, tf_label: str, trade: Dict[str, Any], event: Dict[str, Any]) -> str:
    ...
    return (
        f"✅ {label} HIT — {_dt(display, tf_label)}\n\n"
...
def format_breakeven_message(display: str, tf_label: str, trade: Dict[str, Any], event: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"(58% was already banked earlier at Target 1 and Target 2).\n"
...
def format_trailing_stop_message(display: str, tf_label: str, trade: Dict[str, Any], event: Dict[str, Any]) -> str:
    direction = "LONG" if trade["side"] == "BUY" else "SHORT"
    return (
        f"🔒 TRAILING STOP HIT — {_dt(display, tf_label)}\n\n"
...
def format_runner_stop_message(display: str, tf_label: str, trade: Dict[str, Any], event: Dict[str, Any]) -> str:
    ...
    return (
        ...
        f"All 4 targets were already banked on this {direction} trade — the final 20% "
