        ...
        try:
            ...
            if not best_trade:
                ...
                if ev["attempts"] >= FORWARDED_RESULT_MAX_ATTEMPTS:
                    ...
                    # 🔴 قبلاً فقط همین لاگ بالا بود - که در عمل هیچ‌کس نمی‌دید (لاگ Actions
                    # روتین رصد نمی‌شه). یعنی وقتی یک پیامِ نتیجه به هر دلیلی (مثلاً منبع
                    # فرمت رو کمی عوض کرده، یا پیامِ سیگنال ورودی‌اش اصلاً رهگیری نشده بود)
                    # تطبیق پیدا نمی‌کرد، همون معامله برای همیشه در «معاملات باز» گیر
                    # می‌کرد - هیچ‌وقت به trade_history نمی‌رفت (باعث توقف رشد تعداد
                    # سیگنال‌ها در /results) و نتیجه‌اش هم گزارش نمی‌شد - دقیقاً «نتایج دقیق
                    # و در لحظه گزارش نمی‌شه». الان به ادمین هم اطلاع داده می‌شه، تا این‌بار
                    # واقعاً دیده بشه.
                    notify_admin(
                        f"forwarded_result_unmatched:{ticker}",
                        Exception(f"A forwarded result message for {ticker} (kind={ev.get('kind')}, entry hint "
                                  f"{entry_hint}) never matched any open forwarded-tracked trade after "
                                  f"{ev['attempts']} scan cycles — giving up. If a trade for {ticker} is stuck "
                                  f"showing as 'open' when it has actually closed, this is likely why; it will need "
                                  f"to be resolved manually (e.g. re-forward the result, or use an admin command "
                                  f"to close it)."),
                    )
                continue
...
def process_admin_close_requests():
    ...
    می‌کنه؛ همینجا (candle_engine.py، صاحب واقعی candle_state.json) با کلید دقیق پیدا و بسته
    می‌شه، با close_type مشخص «admin_override» - تا در گزارش‌ها همیشه واضح باشه این نتیجه از
    قیمت واقعی محاسبه نشده، دستی توسط ادمین ثبت شده."""
    ...
    for r in pending:
        ...
        t = s.get("open_trade") if s else None
        if not t or t.get("closed"):
            r["status"] = "not_found"
            notify_admin(
                f"admin_close_not_found:{key}",
                Exception(f"/admin_close_trade requested key={key} but no matching OPEN trade was found "
                          f"(already closed, or the key doesn't exist). Double-check the key with the "
                          f"stale-trade alert message it came from."),
            )
            continue
        ...
        try:
            log_trade_result(t.get("symbol", "?"), t.get("tf", "manual"), t,
                              final_r_override=(final_r if final_r is not None else 0.0))
        except Exception as e:
            logger.error(f"log_trade_result failed for admin close of {key}: {e}")
        r["status"] = "applied"
        changed = True
        logger.info(f"Admin-closed stuck trade {key} (final_r={final_r if final_r is not None else 0.0})")
...
def check_stale_open_trades(candle_states: Dict[str, Any]) -> None:
    ...
    for key, s in candle_states.items():
        ...
        notify_admin(
            f"stale_open_trade:{key}",
            Exception(f"{t.get('display', key)} {t.get('side', '?')} (key={key}, {kind}) has been open for "
                      f"{age_hours}h without resolving — likely stuck due to a missed/unmatched result rather "
                      f"than a genuinely still-open trade. Needs a manual look (check the real price against "
                      f"entry/stop/targets, and close it manually if it should already be closed)."),
        )
