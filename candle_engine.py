...
def git_commit_and_push():
    ...
    pull --rebase (با retry فوری روی برخورد push) که subscription_bot.py هم استفاده می‌کنه،
    تا این دو دیگه هیچ‌وقت دو نسخه‌ی مستقل/drift‌شده از همین منطق نداشته باشن (توضیح کامل در
    docstring خودِ shared_git_sync.py و کامنت بالای importش در این فایل)."""
...
def format_daily_report_message(date_str: str, trades: List[Dict[str, Any]]) -> str:
    ...
    for h in trades:
        ...
        # log_trade_result بالاتر حالا آن را هم ذخیره می‌کند) به‌عنوان جایگزین استفاده
        # می‌شه - دقیقاً همون fallback که subscription_bot.py برای /results استفاده می‌کنه؛
        # فقط وقتی واقعاً هیچ تایم‌فریمی معلوم نیست به «Manual» می‌افته.
        tf = h.get("tf")
        if not tf or tf == "manual":
            tf = h.get("logical_tf") or "Manual"
