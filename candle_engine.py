...
TIMEFRAMES = {
    ...
    # ⚠️ تایم‌فریم 1d طبق درخواست شما کامل حذف شد تا تمرکز/بودجه فقط روی تایم‌فریم‌های
    # پایین‌تر باشه. صادقانه: چون 1d خودش فقط ۱ درخواست در روز مصرف می‌کرد، حذفش بودجه‌ی
    # قابل توجهی آزاد نمی‌کنه (فقط ~۲ درخواست/روز برای هر دو نماد) - فایده‌ی اصلیش سادگی و
    # کاهش نویز/حواس‌پرتی سیگنال‌های خیلی کند-تایم‌فریمه، نه سرعت بیشتر روی 1m/5m.
}
...
# جمع تخمینی: (120+144+96+24+6) × ۲ نماد ≈ ۷۸۰ درخواست/روز، زیر سقف ۸۰۰
# ⚠️ ۱۵m/1h/4h همین الان هم دقیقاً به‌اندازه‌ی فاصله‌ی بسته‌شدن کندل خودشون چک می‌شن -
...
# همزمان با 1m جا نمی‌شد. اگه بعد از دیپلوی، از داشبورد Twelve Data دیدید هنوز به سقف ۸۰۰
# نزدیک نیستید، می‌شه 5m رو هم کمی بیشتر فشرده کرد.
...
POSITION_PLAN_LINES = (
    "\n\n📐 <b>Position plan (RiskRivard)</b>\n"
    "🎯 Target 1 (1R): close 20% · stop → Entry\n"
    "🎯 Target 2 (2R): close 30% · stop → Target 1\n"
    "🎯 Target 3 (4R): close 15% · stop → Target 2\n"
    "🎯 Target 4 (6R): close 10% · trailing stop starts on the remaining 25% (\"Runner\"), "
    "1.5R behind the peak price — no fixed cap"
)
