...
def next_signal_id(candle_states: Optional[Dict[str, Any]] = None) -> str:
    ...
    شناسه، اون رو در برابر همه‌ی معاملاتِ *هنوز-باز* چک می‌کنه؛ اگه تصادم پیدا شد، تا رسیدن به
    یک شناسه‌ی واقعاً آزاد جلو می‌ره و یک هشدار به ادمین می‌فرسته (چون خودِ وجودِ تصادم یعنی یک
    مشکل جدی‌تر - احتمالاً دو نمونه‌ی هم‌پوشانِ این اسکریپت هم‌زمان در حال اجرا هستن - که چک
    concurrency در ورک‌فلوی GitHub Actions باید جلوش رو بگیره، نه فقط این تابع).
    ⚠️ این چک فقط معاملاتِ هنوز-باز رو می‌بینه، نه کل trade_history.json (که در این نقطه از
    کد در دسترس نیست) - پس یک تصادم با یک معامله‌ی از قبل *بسته‌شده* رو تشخیص نمی‌ده. اگه بعد
    از این اصلاح هم دوباره یک #TC_XXXXXX تکراری دیدید، تقریباً قطعاً یعنی دو نمونه‌ی هم‌پوشانِ
    این اسکریپت هم‌زمان اجرا می‌شن - نه یک باگِ دیگه در همین تابع - و باید مستقیم سراغِ
    concurrency ورک‌فلوی GitHub Actions این ریپو رفت."""
    ...
    if candle_states is not None:
        while _signal_id_in_use(candidate, candle_states):
            n += 1
            skipped += 1
            candidate = f"{SIGNAL_ID_PREFIX}-{n:06d}"
        if skipped:
            ...
            notify_admin("signal_id_collision", Exception(
                ...
                f"certainly means two overlapping instances of candle_engine.py are running at once "
                f"(check the `concurrency` block in this repo's GitHub Actions workflow)."))
...
def process_symbol_timeframe(symbol: str, display: str, tf_key: str, tf_cfg: Dict[str, Any],
    ...
    for idx, k in enumerate(new_candles):
        ...
        if sig and idx == last_new_idx:
            ...
            trade = None if tf_key in DISABLED_AUTO_TIMEFRAMES else open_new_trade(sig, symbol=symbol, tf_key=tf_key, display=display)
