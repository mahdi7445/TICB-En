...
def _id_line(trade: Dict[str, Any]) -> str:
    """خط شناسه‌ی یکتای سیگنال («🆔 TC-004821») - در پیام ورود و هر پیام خروج/تارگت/استاپ
    نشون داده می‌شه، تا هرکسی که کانال رو دنبال می‌کنه (یا ادمین، برای /admin_close_trade)
    بتونه دقیقاً به همون یک سیگنالِ مشخص اشاره کنه - حتی وقتی چند سیگنالِ پی‌درپی روی یک
    نماد/تایم‌فریم جابه‌جا می‌شن. اگه به هر دلیلی (رکورد خیلی قدیمی، قبل از این فیچر) شناسه
    موجود نباشه، رشته‌ی خالی برمی‌گردونه - هیچ خط اضافه‌ای به پیام اضافه نمی‌شه."""
        ...
        if not t:
            ...
            notify_admin(
                ...
                Exception(f"/admin_close_trade requested key={key} but no matching OPEN trade was found "
                          f"(already closed, or the key/signal_id doesn't exist). Double-check against the "
                          f"stale-trade alert message it came from — prefer the signal_id (e.g. TC-004821) "
                          f"shown there over the older symbol|timeframe key when both are available."),
            )
            continue
