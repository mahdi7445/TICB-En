...
def git_commit_and_push():
    """هر چند ثانیه یک‌بار تغییرات data/ رو کامیت و پوش می‌کنه، و - مهم - همیشه یک `git pull`
    هم انجام می‌ده حتی اگه خودِ این اسکریپت چیزی برای کامیت نداشته باشه. منطق واقعی
    (add/commit/pull --rebase/push + retry) توی shared_git_sync.py هست - مشترک بین این
    اسکریپت و subscription_bot.py، دقیقاً به همون دلیلی که shared_risk_config.py جدا شد:
    این منطق قبلاً عیناً و دستی توی هر دو فایل کپی شده بود.

    ⚠️ چرا الان retry فوری اضافه شد (علت خطای «git push failed ... Updates were rejected»):
    این اسکریپت و subscription_bot.py هرکدوم در یک workflow/checkout کاملاً جدا، به‌صورت
    پیوسته برای ~۵ ساعت و ۲۰ دقیقه، به همون شاخه push می‌کنن (این هر ۴۵ ثانیه، اون هر ۱۲۰
    ثانیه). روی این تعداد push، برخورد (non-fast-forward) قطعاً پیش میاد - اتفاق عادی این
    معماریه. قبلاً retry فقط توی «دور بعدی» (۴۵ ثانیه بعد) انجام می‌شد؛ الان shared_git_sync
    همون‌جا تا ۴ بار با تاخیر تصادفی retry می‌کنه، پس اکثر برخوردها اصلاً به notify_admin
    نمی‌رسن - فقط اگه واقعاً چند بار پشت‌سرهم شکست بخوره (یعنی احتمالاً یک مشکل واقعی، نه
    صرفاً برخورد گذرا با subscription_bot.py) به ادمین اطلاع داده می‌شه."""
    try:
        from shared_git_sync import sync_data_dir
        repo_dir = os.path.dirname(os.path.abspath(__file__))

        def on_error(kind: str, message: str):
            logger.warning(f"[git] {kind}: {message}")
            notify_admin(f"git_{kind}", Exception(f"candle_engine.py: {message}"))
