...
def git_commit_and_push():
    ...
    ⚠️ باگ اول: pull فقط زمانی اجرا می‌شد که این اسکریپت خودش تغییر محلی برای کامیت داشت -
    یعنی سیگنال‌های دستی/فوروارد‌شده‌ای که subscription_bot.py (یک workflow کاملاً جدا با
    چک‌اوت گیت جداگانه) می‌نوشت، تا هر وقت که این اسکریپت خودش state تغییر می‌داد دیده
    نمی‌شدن. رفع شد: pull هر دور بی‌قید‌وشرط اجرا می‌شه.

    ⚠️ باگ دوم (علت گزارش «سیگنال باز توی کانال هست ولی توی /results نیست»): اگه
    `git pull --rebase` شکست می‌خورد، ریپو توی حالت نیمه‌rebase می‌موند، و push فقط وقتی
    امتحان می‌شد که «همین دور» خودش چیزی commit کرده بود - یعنی یک push ناموفق می‌تونست
    برای همیشه بدون retry بمونه. رفع شد: rebase ناموفق صراحتاً abort می‌شه، و push همیشه
    (نه فقط با تغییر محلیِ همین دور) امتحان می‌شه.

    ⚠️ باگ سوم (علت پیام خطای «git_commit_and_push_exception / git add ... returned
    non-zero exit status 128»): دو مشکل با هم بودن -
    (۱) خودِ خطا: exit code 128 روی `git add` تقریباً همیشه یکی از این‌هاست: مسیر ریپو نیست،
    خطای معروف «detected dubious ownership» (خیلی رایج روی GitHub Actions با گیت جدید، وقتی
    مالکیت پوشه‌ی checkout با کاربر اجراکننده دقیقاً یکی تشخیص داده نشه)، یک `.git/index.lock`
    جامونده از یک اجرای قطع‌شده، یا خرابی ایندکس. رفع پیشگیرانه‌ی رایج‌ترین حالت: اول تابع
    `git config --global --add safe.directory` برای این مسیر ست می‌شه - idempotent و بی‌خطر
    حتی اگه علت این نبوده باشه.
    (۲) مشکل تشخیصی: چون `check=True` بدون capture خروجی استفاده می‌شد، پیامی که به ادمین
    می‌رسید فقط «returned non-zero exit status 128» بود - متن واقعی خطای گیت (stderr) اصلاً
    ذخیره/نمایش نمی‌شد. رفع شد: همه‌ی دستورهای گیت با capture_output اجرا می‌شن و در صورت
    شکست، stderr واقعی مستقیم توی پیام notify_admin می‌ره - از این به بعد هر بار شکستی پیش
    بیاد، دقیقاً معلومه چرا، نه فقط یک کد خروجی خام."""
    try:
        import subprocess
        ...
        def _err_tail(res, n=400) -> str:
            txt = (res.stderr or res.stdout or "").strip()
            return txt[-n:] if txt else "(no stderr/stdout captured)"

        # رفع پیشگیرانه‌ی رایج‌ترین علت exit 128: مالکیتِ پوشه‌ی checkout با کاربر جاری
        # «مشکوک» تشخیص داده می‌شه و گیت از اجرای هر دستوری امتناع می‌کنه، مگر این‌که صریحاً
        # به‌عنوان safe.directory معرفی بشه. idempotent و بی‌خطر - حتی اگه علت چیز دیگه‌ای
        # باشه، این خط چیزی خراب نمی‌کنه.
        subprocess.run(["git", "config", "--global", "--add", "safe.directory", repo_dir],
                        capture_output=True, text=True)

        if not os.path.isdir(os.path.join(repo_dir, ".git")):
            logger.error(f"[git] {repo_dir} does not look like a git repository (.git missing) - skipping sync")
            notify_admin("git_not_a_repo", Exception(
                f"candle_engine.py: {repo_dir} has no .git directory — state can't be committed/pushed "
                f"at all until the checkout is fixed. Needs a manual look at the workflow's checkout step."))
            return

        # اگه یک اجرای قبلی وسط commit/rebase قطع شده باشه، ممکنه index.lock جامونده باشه که
        # هر دستور بعدی رو با exit 128 رد می‌کنه. چون هر اجرای Actions روی یک runner تازه‌ست،
        # این نادره، ولی چک‌کردنش بی‌ضرره و مجانیه.
        lock_file = os.path.join(repo_dir, ".git", "index.lock")
        if os.path.exists(lock_file):
            try:
                os.remove(lock_file)
                logger.warning("[git] removed a stale .git/index.lock before retrying")
            except OSError as e:
                logger.error(f"[git] found .git/index.lock but couldn't remove it: {e}")

        add_res = run(["git", "add", "data/"])
        if add_res.returncode != 0:
            logger.error(f"[git] git add failed: {_err_tail(add_res)}")
            notify_admin("git_add_failed", Exception(
                f"candle_engine.py: git add data/ failed (exit {add_res.returncode}): {_err_tail(add_res)}"))
            return

        result = run(["git", "diff", "--cached", "--quiet"])
        has_local_changes = (result.returncode != 0)
        if has_local_changes:
            commit_res = run(["git", "commit", "-m", "update candle/trade state [skip ci]"])
            if commit_res.returncode != 0:
                logger.error(f"[git] git commit failed: {_err_tail(commit_res)}")
                notify_admin("git_commit_failed", Exception(
                    f"candle_engine.py: git commit failed (exit {commit_res.returncode}): "
                    f"{_err_tail(commit_res)}"))
                return

        pull = run(["git", "pull", "--rebase"])
        if pull.returncode != 0:
            run(["git", "rebase", "--abort"])
            logger.warning(f"[git] pull --rebase failed/conflict — aborted, will retry next cycle: {_err_tail(pull)}")
            notify_admin(
                "git_pull_rebase_conflict",
                Exception(f"candle_engine.py: git pull --rebase failed and was aborted ({_err_tail(pull)}). "
                          f"Any state changes this cycle (open trades, results, etc.) stayed committed "
                          f"locally and could NOT be pushed yet — they'll retry automatically next cycle, "
                          f"but won't show up in /results until this resolves. If it keeps repeating, "
                          f"there's likely a real merge conflict in data/ needing a manual look."),
            )
            return

        # همیشه push رو امتحان کن - نه فقط وقتی همین دور چیزی برای commit داشت (وگرنه یک push
        # ناموفق می‌تونه برای همیشه بدون retry بمونه - همون علت باگ دوم بالا).
        push = run(["git", "push"])
        if push.returncode != 0:
            logger.warning(f"[git] push failed — will retry next cycle: {_err_tail(push)}")
            notify_admin(
                "git_push_failed",
                Exception(f"candle_engine.py: git push failed ({_err_tail(push)}). Any "
                          f"committed-but-unpushed state (open trades, manual/forwarded signals, etc.) "
                          f"will NOT show up in subscription_bot.py's /results until this succeeds — "
                          f"will keep retrying automatically every cycle."),
            )
...
def format_daily_report_message(date_str: str, trades: List[Dict[str, Any]]) -> str:
    ...
    for h in trades:
        # سیگنال‌های دستی/فوروارد‌شده (tf=="manual") قبلاً همه زیر یک برچسبِ خامِ «Manual»
        # جمع می‌شدن، حتی وقتی خودِ پیام منبع تایم‌فریم مشخصی (مثلاً «1M») اعلام کرده بود -
        # چون اون اطلاعات (logical_tf) قبلاً موقع بسته‌شدن معامله ذخیره نمی‌شد. الان اگه
        # logical_tf موجود باشه (سیگنال‌های تازه‌تر - رکوردهای قدیمی‌تر این فیلد رو ندارن)
        # همون رو به‌عنوان کلید گروه‌بندی استفاده می‌کنیم؛ فقط وقتی واقعاً هیچ تایم‌فریمی
        # معلوم نیست (سیگنال دستیِ صرف بدون هیچ برچسبی) زیر «Manual» جمع می‌شه.
        tf_key = h.get("tf")
        if not tf_key or tf_key == "manual":
            tf_key = h.get("logical_tf") or "Manual"
        by_tf.setdefault(tf_key, []).append(h)
    lines.append("\n<b>By timeframe</b>")
...
def log_trade_result(symbol: str, tf_key: str, trade: Dict[str, Any], final_r_override: Optional[float] = None):
    ...
    ⚠️ چرا «logical_tf» هم ذخیره می‌شه (علت گزارش «By timeframe توی /results هیچی نشون
    نمی‌ده»): برای سیگنال‌های دستی/فوروارد‌شده (از جمله رله‌ی خودکار کانال آلتکوین -
    handle_altcoin_relay_post - که الان عملاً بیشترین حجم سیگنال‌هاست)، این تابع همیشه با
    tf_key="manual" صدا زده می‌شه (برای این‌که این سیگنال‌ها به‌درستی زیر «Manual» جدا از
    تایم‌فریم‌های خودکار BTC/ETH دیده بشن). ولی process_manual_signals تایم‌فریمِ واقعیِ
    اعلام‌شده در خودِ پیام (مثلاً «1M») رو هم روی trade["logical_tf"] نگه می‌داشت - فقط برای
    چک تکرار/جهت‌مخالفِ بین‌منبعی موقع باز شدن معامله، و هیچ‌وقت به trade_history.json ذخیره
    نمی‌شد؛ یعنی وقتی معامله بسته می‌شد، این اطلاعات برای همیشه گم می‌شد. subscription_bot.py
    توی format_results_message هر معامله‌ای با tf=="manual" رو کلاً از بخش «By timeframe»
    حذف می‌کرد - و چون تقریباً همه‌ی معاملاتِ بسته‌شده‌ی اخیر از همین مسیر (رله‌ی VIP) می‌آن،
    همه‌شون tf=="manual" داشتن، پس اون بخش همیشه خالی می‌موند. الان logical_tf هم ذخیره
    می‌شه تا subscription_bot.py بتونه ازش به‌عنوان جایگزین برای گروه‌بندی استفاده کنه، بدون
    این‌که چیزی توی چیدمان «By symbol» / نمایش کلی تغییر کنه."""
    os.makedirs(DATA_DIR, exist_ok=True)
    history = []
    if os.path.exists(TRADE_HISTORY_FILE):
        try:
...
def run_scan_cycle():
    ...
    ⚠️ باگ پیدا‌شده (علت گزارش «سیگنال‌های فوروارد‌شده نه توی معاملات باز نه توی نتایج/گزارش
    روزانه دیده نمی‌شن، درحالی‌که سیگنال‌های خودکار درستن و هیچ هشدار گیتی هم نیومده»): اگه
    process_manual_signals() یا process_forwarded_results() به هر دلیلی (مثلاً یک فرمت
    غیرمنتظره در یک سیگنال دستیِ خاص) وسط کارش استثنا بگیره، کل تابع همون لحظه متوقف می‌شد -
    یعنی نه manual_signals.json/forwarded_results_queue.json آپدیت می‌شد، نه save_state صدا
    زده می‌شد؛ فقط `logger.error` (که فقط توی لاگ خام Actions می‌ره، جایی که طبق همین کامنت‌ها
    «کسی رصدش نمی‌کنه») - بدون هیچ notify_admin. نتیجه: اگه این استثنا هر دور تکرار بشه (چون
    علتش دائمیه، نه گذرا)، سیگنال‌های دستی/فوروارد برای همیشه pending می‌مونن و هیچ‌وقت باز/
    بسته نمی‌شن، بدون این‌که هیچ‌کس متوجه بشه. رفع شد با دو لایه:
    (۱) این wrapping بیرونی حالا notify_admin هم می‌کنه (با traceback کامل، نه فقط پیام خطا)،
    (۲) مهم‌تر - داخل خودِ process_manual_signals/process_forwarded_results، هر آیتم داخل try/
    except جدای خودش پردازش می‌شه؛ یعنی از این به بعد یک سیگنال خراب فقط خودش رد می‌شه (با
    notify_admin دقیق)، نه این‌که کل batch (بقیه‌ی سیگنال‌های سالم دستی/فوروارد اون دور) رو
    هم با خودش متوقف کنه."""
    try:
        ...
        notify_admin("process_manual_signals_crashed", Exception(
            ...
            f"signals were processed this cycle (none opened, none closed) - they'll stay "
    ...
    try:
        ...
        notify_admin("process_forwarded_results_crashed", Exception(
            ...
            f"result messages were applied this cycle - open forwarded trades won't update or "
