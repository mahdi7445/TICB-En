...
def format_daily_report_message(date_str: str, trades: List[Dict[str, Any]]) -> str:
    ...
    for h in trades:
        # سیگنال‌های دستی/فوروارد‌شده‌ای که تایم‌فریمِ منبع تشخیص داده نشد، زیر «Manual» جمع
        # می‌شن - نه یک برچسب خام مثل None که برای کاربر بی‌معنیه
        by_tf.setdefault(h.get("tf") or "Manual", []).append(h)
    lines.append("\n<b>By timeframe</b>")
