...
TF_CHECK_INTERVAL_SECONDS = {
    "1m": 15 * 60,   # با فقط ۲ نماد، بودجه اجازه می‌ده تنگ‌تر از قبل باشه (قبلاً ۳۰ دقیقه بود)
    "5m": 10 * 60,   # قبلاً ۲۰ دقیقه بود
    "15m": 15 * 60,
    "1h": 60 * 60,
    "4h": 4 * 60 * 60,
    "1d": 24 * 60 * 60,
}
# جمع تخمینی: (96+144+96+24+6+1) × ۲ نماد ≈ ۷۳۴ درخواست/روز، زیر سقف ۸۰۰
...
def git_commit_and_push():
    """هر چند دقیقه یک‌بار تغییرات data/ رو کامیت و پوش می‌کنه - چون این اسکریپت الان ساعت‌ها
    پیوسته اجرا می‌مونه، دیگه نمی‌شه فقط منتظر کامیت نهایی workflow موند."""
    try:
        import subprocess
...
def format_daily_report_message(date_str: str, trades: List[Dict[str, Any]]) -> str:
    ...
    for h in trades:
        ...
        tf_label = h.get("tf", "?")
        lines.append(f"{arrow} {h['symbol']} · {tf_label} — {h.get('close_reason', h.get('close_type','?'))} ({h['final_r']:+.2f}R)")
    return "\n".join(lines)
...
def process_manual_signals():
    """سیگنال‌هایی که ادمین با /admin_manual_signal در ربات اشتراک ثبت کرده رو می‌خونه،
    دقیقاً با همون فرمت سیگنال‌های خودکار (استاپ + ۴ تارگت) در کانال پست می‌کنه، و به همون
    سیستم رهگیری (WebSocket برای بیت‌کوین/اتریوم، REST به‌عنوان پشتیبان) اضافه می‌کنه تا
    نتیجه‌ش هم داخل /results حساب بشه."""
    ...
    for m in pending:
        ...
        # برای اینکه چارت هم بشه ساخت، چند کندل اخیر رو می‌گیریم (تایم‌فریم ۱۵ دقیقه، فقط برای نمایش)
        hist = []
        try:
            candles = fetch_closed_klines(symbol, 80, "15min", 15 * 60)
            if candles:
                ...
                hist = [{"o": c["o"], "h": c["h"], "l": c["l"], "c": c["c"], "ema7": e, "dt_ms": c["open_time"]}
                        for c, e in zip(candles, ema7_vals)]
        except Exception as e:
            logger.warning(f"Could not build chart history for manual signal: {e}")
...
# باید بعد از دیپلوی، از لاگ Actions تایید بشه. اگه وصل نشه، رهگیری REST بالا (هرچند کندتر)
# همچنان به‌عنوان پشتیبان برای بیت‌کوین/اتریوم هم کار می‌کنه.
...
def start_price_stream(latest_prices: Dict[str, float], price_lock, stop_event) -> None:
    """در یک ترد جداگانه اجرا می‌شه؛ تا وقتی stop_event ست نشده، مدام سعی می‌کنه وصل بمونه
    و با هر تیک قیمت، latest_prices رو آپدیت می‌کنه (بدون هیچ هزینه‌ی درخواست API)."""
    try:
        import websocket
    except ImportError:
    ...
    def on_open(ws):
        logger.info(f"🔌 WebSocket connected — subscribing to {product_ids}")
    ...
    while not stop_event.is_set():
        try:
            ...
            backoff = 5  # موفق بود، بعد از قطع طبیعی دوباره با تاخیر کم امتحان کن
        except Exception as e:
            logger.error(f"WebSocket connection failed: {e}")
        if stop_event.is_set():
            break
        time.sleep(backoff)
        backoff = min(backoff * 2, 60)  # backoff نمایی تا سقف ۶۰ ثانیه
