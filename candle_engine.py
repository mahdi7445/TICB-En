...
def run_scan_cycle():
    """یک دور کامل اسکن: سیگنال‌های جدید (هر ۳ نماد) + رهگیری زنده‌ی REST برای معاملات باز
    (این رهگیری REST همچنان برای طلا لازمه چون WebSocket رایگان براش وجود نداره، و برای
    بیت‌کوین/اتریوم هم به‌عنوان پشتیبان، برای مواقعی که اتصال WebSocket قطع باشه)."""
    run_start = time.time()
...
def check_ws_open_trades(latest_prices: Dict[str, float], price_lock) -> None:
    """معاملات باز بیت‌کوین/اتریوم رو با آخرین قیمتِ رسیده از WebSocket چک می‌کنه - این تابع
    هر چند ثانیه یک‌بار در حلقه‌ی اصلی صدا زده می‌شه، بدون هیچ هزینه‌ی API."""
    with price_lock:
