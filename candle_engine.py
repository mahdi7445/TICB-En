...
from shared_risk_config import (
    ...
    VALID_TIMEFRAME_LABELS, TARGET_ACTION_LINE, format_signal_price as _shared_format_signal_price,
)
...
# 🔴 اضافه شد: طبق بک‌تستِ کاربر («1m از اول سوددهی نداشت، 4h بیشترین سود رو می‌داد» -
# تأیید‌شده با ۵۰ معامله‌ی زنده‌ی اخیر هم: 1m=۳۰ معامله، ۴۷٪ برد، -۵.۴R در برابر بقیه‌ی
# تایم‌فریم‌ها که مثبت بودن) - یک راه کاملاً برگشت‌پذیر برای خاموش/روشن‌کردنِ صدورِ سیگنالِ
# خودکارِ جدید روی یک تایم‌فریمِ خاص، بدون تغییر در بقیه‌ی منطق. فقط جلوی *باز شدنِ معامله‌ی
# جدیدِ خودکار* رو می‌گیره (هر دو نقطه‌ی open_new_trade داخل process_symbol_timeframe پایین‌تر)
# - معاملاتِ از قبل باز (و سیگنال‌های دستی/فوروارد‌شده، که اصلاً از این مسیر رد نمی‌شن)
# دقیقاً مثل همیشه رهگیری/بسته می‌شن، هیچ اثرِ جانبی‌ای روی بقیه‌ی سیستم نداره.
#
# پیش‌فرض «1m» است (طبق همین بک‌تست). برای تغییرش از طریق Secret این ریپو
# (DISABLED_AUTO_TIMEFRAMES) - و **فقط** از همین طریق، چون این متغیر در کل کد فقط همین یک‌بار
# در پایین از env خونده می‌شه و بعدش هیچ‌جای دیگه (نه از subscription_bot.py، نه از هیچ دستور
# ادمینی) دوباره نوشته نمی‌شه:
...
# 🔴 رفعِ باگِ واقعی: قبلاً "خالی بذارید" (چه با حذفِ کامل Secret چه با ست‌کردنش به مقدار
# خالی) هر دو به یک نتیجه می‌رسیدن (پیش‌فرضِ "1m") - یعنی هیچ راهی برای گفتنِ صریحِ «هیچ‌کدام
# رو غیرفعال نکن» نبود؛ چون GitHub Actions یک Secretِ تنظیم‌نشده رو هم دقیقاً همون رشته‌ی
# خالیِ یک Secretِ عمداً-خالی‌گذاشته‌شده پاس می‌ده - این دو حالت از دیدِ خودِ کد قابل‌تفکیک
# نبودن. حالا با یک مقدارِ صریح ("none") این ابهام کاملاً برطرف شده.
...
def next_signal_id(candle_states: Optional[Dict[str, Any]] = None) -> str:
    """🔴 رفعِ باگِ واقعیِ «دو سیگنالِ کاملاً متفاوت با یک شناسه‌ی یکسان» (دیده‌شده در کانال:
    یک SHORT روی ETH با #TC_000002 و ساعت‌ها بعد یک SHORT کاملاً متفاوت روی BTC هم با همون
    #TC_000002). این تابع تا قبل از این نسخه، صرفاً به مقدار فایلِ signal_id_counter.json
    اعتماد می‌کرد - اگه بین دو فراخوانی، این فایل به هر دلیلی (محتمل‌ترین سناریو: دو نمونه‌ی
    هم‌پوشانِ همین اسکریپت روی دو runner جداگانه‌ی GitHub Actions - نگاه کنید نکته‌ی زیر)
    به یک مقدارِ قدیمی‌تر برگرده، تابع بدون هیچ چکی همون شماره‌ی تکراری رو دوباره برمی‌گردوند.
    الان - دقیقاً مثل گاردی که preferred_signal_id (بالاتر) از قبل داشت - قبل از برگردوندنِ
    شناسه، اون رو هم در برابر همه‌ی معاملاتِ *هنوز-باز* و هم در برابر همه‌ی معاملاتِ *از قبل
    بسته‌شده* (trade_history.json) چک می‌کنه؛ اگه تصادم پیدا شد، تا رسیدن به یک شناسه‌ی واقعاً
    آزاد جلو می‌ره و یک هشدار به ادمین می‌فرسته (چون خودِ وجودِ تصادم یعنی یک مشکل جدی‌تر -
    احتمالاً دو نمونه‌ی هم‌پوشانِ این اسکریپت هم‌زمان در حال اجرا هستن، یا شمارنده به‌خاطر یک
    بازگردانیِ گیت [مثلاً hard-reset بعد از یک تعارضِ پایدار - نگاه کنید shared_git_sync.py]
    عقب رفته - که چک concurrency در ورک‌فلوی GitHub Actions باید جلوی سناریوی اول رو بگیره،
    نه فقط این تابع).
    🔴 اضافه شد: چکِ trade_history.json. قبلاً این گارد فقط معاملاتِ هنوز-باز رو می‌دید - یعنی
    اگه شمارنده عقب می‌رفت، محتمل‌ترین برخورد (دقیقاً با یکی از همون معاملاتی که در همون
    بازه‌ی زمانیِ گم‌شده به‌تازگی هم باز و هم بسته شده بودن) اصلاً تشخیص داده نمی‌شد - چون
    همون معامله دیگه در candle_states نیست (بسته شده و از اونجا حذف شده)، فقط در
    trade_history.json. الان هر دو منبع چک می‌شن.
    ⚠️ این تابع فقط زمانی این چک کامل رو انجام می‌ده که caller خودش candle_states رو پاس بده
    (هر سه محلِ باز کردنِ معامله‌ی جدید - خودکار و دستی/فوروارد‌شده - این کار رو می‌کنن)."""
    ...
    if candle_states is not None:
        closed_ids = _load_closed_signal_ids()
        while _signal_id_in_use(candidate, candle_states, closed_ids):
            n += 1
            skipped += 1
            candidate = f"{SIGNAL_ID_PREFIX}-{n:06d}"
        if skipped:
            ...
            notify_admin("signal_id_collision", Exception(
                f"{skipped} colliding signal_id(s) skipped before minting {candidate} - this almost "
                f"certainly means either two overlapping instances of candle_engine.py running at once "
                f"(check the `concurrency` block in this repo's GitHub Actions workflow), or that "
                f"signal_id_counter.json was reverted to an older value (e.g. by a git hard-reset after "
                f"a persistent rebase conflict)."))
...
# 🔴 اضافه شد: html.escape روی msg در هر لامبدا (قبلاً نبود). چرا الان لازم شد: تا وقتی
# rebase_conflict شاملِ متنِ خامِ خروجیِ گیت بدونِ جزئیاتِ تعارض بود، escape رو خودِ
# notify_admin (مسیرِ پیش‌فرض، بدونِ full_text) انجام می‌داد - ولی همون مسیر طولِ پیام رو هم
# بی‌قید‌وشرط به ۵۰۰ کاراکتر می‌بریدید، که دقیقاً همون بخشِ تازه‌اضافه‌شده (لیستِ فایل‌های
# تعارض‌دار + diff خلاصه‌شون، نگاه کنید shared_git_sync.py::_conflict_summary) رو اغلب کامل
# قطع می‌کرد - یعنی این ویژگیِ جدید برای candle_engine.py عملاً دیده نمی‌شد. حالا on_error
# پایین‌تر این پیام رو با full_text (بدونِ آن سقفِ ۵۰۰ کاراکتری) می‌فرسته - پس escape باید
# همینجا، دستی، انجام بشه (دقیقاً همون الگویی که subscription_bot.py از قبل برای همین دقیقاً
# همین دلیل - جلوگیری از ۴۰۰ Bad Request روی کاراکترهای HTML-مانندِ خروجیِ گیت - دارد).
_GIT_ERROR_MESSAGES = {
    "not_a_repo": lambda msg: f"candle_engine.py: {html.escape(msg)}",
    "add_failed": lambda msg: f"candle_engine.py: git add data/ failed: {html.escape(msg)}",
    "commit_failed": lambda msg: f"candle_engine.py: git commit failed: {html.escape(msg)}",
    "rebase_conflict": lambda msg: (
        f"candle_engine.py: {html.escape(msg)} Won't show up in /results until this resolves."),
    "rebase_stuck_recovered": lambda msg: f"candle_engine.py: {html.escape(msg)}",
    "rebase_stuck_recovery_failed": lambda msg: f"candle_engine.py: {html.escape(msg)}",
    "push_failed": lambda msg: (
        f"candle_engine.py: {html.escape(msg)} Won't show up in subscription_bot.py's /results until this succeeds."),
}


def git_commit_and_push(final: bool = False) -> bool:
    ...
    🔴 اضافه شد: مقدارِ بازگشتیِ bool (قبلاً هیچی برنمی‌گردوند). دلیل و پیامدش (رفعِ باگِ
    «گزارش روزانه دوبار ارسال می‌شه») در docstring خودِ sync_data_dir توضیح داده شده."""
    repo_dir = os.path.dirname(os.path.abspath(__file__))

    def on_error(kind: str, message: str) -> None:
        logger.warning(f"[git] {kind}: {message}")
        text = _GIT_ERROR_MESSAGES.get(kind, lambda m: f"candle_engine.py: {html.escape(m)}")(message)
        if final:
        ...
        # 🔴 اضافه شد: full_text (به‌جای گذاشتنش به مسیرِ پیش‌فرضِ notify_admin) - چون اون
        # مسیر طولِ Error را بی‌قید‌وشرط به ۵۰۰ کاراکتر می‌بُرید، که برای rebase_conflict
        # (که الان شاملِ لیستِ فایل‌های تعارض‌دار + diff خلاصه‌شون هم می‌شه - نگاه کنید
        # shared_git_sync.py::_conflict_summary) اغلب همون جزئیاتِ تازه‌اضافه‌شده رو قطع
        # می‌کرد. escape قبلاً همینجا (بالا، در خودِ _GIT_ERROR_MESSAGES) انجام شده، پس
        # full_text امن است.
        icon = {"rebase_stuck_recovered": "🛠️", "rebase_stuck_recovery_failed": "🛑"}.get(kind, "⚠️")
        full_text = f"{icon} <b>candle_engine.py</b>\n{text}\n{_now_str()}"
        notify_admin(f"git_{kind}", Exception(text), full_text=full_text)

    if final:
        return sync_data_dir(repo_dir, "update candle/trade state [skip ci]", on_error,
...
def maybe_send_daily_report():
    ...
    ⚠️ باگِ اول (قبلاً رفع شد): پرچمِ «امروز فرستاده شد» بلافاصله push نمی‌شد - فقط محلی
    save می‌شد و منتظر دور بعدیِ commit/push دوره‌ای (هر ۲ دقیقه) می‌موند. راه‌حل: (۱) قبل
    از هر تصمیمی یک‌بار pull تازه، و (۲) به‌محض ثبت پرچم، بلافاصله (نه منتظر دور بعدی)
    commit+push - قبل از اینکه اصلاً پیام رو بفرستیم.

    🔴 باگِ دوم (تازه پیدا و رفع شد - دقیقاً چیزی که باعث ارسالِ دوباره‌ی گزارشِ ۲۰۲۶-۰۹-۰۵
    شد): قدمِ «بلافاصله push کن» بالا، اگه خودش (نه فقط تئوری، عملاً) شکست می‌خورد
    (تعارضِ حل‌نشدنی، قطعیِ موقتِ شبکه، ...)، تا همین الان به فراخوان اصلاً اطلاع داده
    نمی‌شد - git_commit_and_push() هیچ‌وقت True/False برنمی‌گردوند. یعنی حتی وقتی این
    pushِ حیاتی واقعاً شکست می‌خورد، کد بدون توقف مستقیم می‌رفت سراغ فرستادنِ پیام؛ پیام با
    موفقیت می‌رفت، ولی پرچمِ «فرستاده شد» فقط روی دیسکِ محلیِ همون یک اجرا می‌موند - هرگز
    به ریموت نمی‌رسید. با هر ری‌استارتِ بعدی (self-chain، هر ~۵ ساعت و ۲۰ دقیقه، با یک
    checkout تازه از ریموت که این پرچم رو نداره)، شرط «امروز قبلاً فرستاده شده؟» دوباره
    False می‌شد و گزارشِ همون روز دوباره فرستاده می‌شد.

    رفع: sync_data_dir/git_commit_and_push الان True/False برمی‌گردونن. اگه دقیقاً همین
    pushِ حیاتی شکست بخوره، پرچمِ محلی رو *برمی‌گردونیم* (rollback - انگار هیچ‌وقت ثبت
    نشده) و بدونِ فرستادنِ پیام، به‌سادگی از تابع خارج می‌شیم؛ چون این تابع هر چند ثانیه
    یک‌بار در حلقه‌ی اصلی صدا زده می‌شه، دورِ بعدی از صفر (با یک pull تازه) دوباره تلاش
    می‌کنه - تا وقتی push واقعاً موفق بشه، نه پیام فرستاده می‌شه نه ابهامی باقی می‌مونه."""
    ...
    if not git_commit_and_push():
        # 🔴 push شکست خورد - پرچمِ محلی رو برمی‌گردونیم (rollback) تا این پروسه یا اجرای
        # بعدی، دورِ بعدی دوباره از صفر تلاش کنه؛ همینجا بدونِ فرستادنِ پیام خارج می‌شیم.
        prev_sent_dates = [d for d in sent_dates if d != today_str]
        _save_daily_report_log(prev_sent_dates)
        logger.warning(f"📋 Daily report for {today_str}: failed to durably push the 'sent' flag — "
                        f"NOT sending yet (to avoid an unpushed flag that could cause a duplicate send "
                        f"after a restart). Will retry on the next cycle.")
        return
...
def maybe_send_weekly_report():
    ...
    توضیح کامل چرایی‌اش بالای maybe_send_daily_report، شاملِ رفعِ باگِ اخیرِ «push شکست
    می‌خورد ولی کسی خبردار نمی‌شد» (git_commit_and_push الان True/False برمی‌گردونه)."""
    now = datetime.now(timezone.utc)
    ...
    if not git_commit_and_push():  # فوری پوش - قبل از تلاش برای ارسال پیام
        # 🔴 همون فیکسِ maybe_send_daily_report: اگه این push شکست بخوره، پرچمِ محلی رو
        # برمی‌گردونیم و بدونِ فرستادنِ پیام خارج می‌شیم - دورِ بعدی دوباره تلاش می‌کنه.
        prev_sent_weeks = [w for w in sent_weeks if w != week_id]
        _save_weekly_report_log(prev_sent_weeks)
        logger.warning(f"📈 Weekly report for {week_id}: failed to durably push the 'sent' flag — "
                        f"NOT sending yet. Will retry on the next cycle.")
        return
...
def notify_admin(context: str, error: Exception, reply_markup: Optional[Dict[str, Any]] = None,
    ...
    # 🔴 رفعِ باگِ بحرانی: قبلاً context و پیامِ error مستقیم (بدون escape) داخل تگ‌های HTML
    # (<code>) جاسازی می‌شدن. چون این پیام‌ها با parse_mode=HTML فرستاده می‌شن، اگه متنِ
    # error (که می‌تونه از هر جایی بیاد - خروجیِ خامِ گیت مثل "<conflicted_files>"، یک
    # traceback پایتون که تقریباً همیشه "<module>" داره، یا حتی یک ورودیِ کاربر) شاملِ کاراکترِ
    # '<' یا '&' باشه، تلگرام کل پیام رو با خطای 400 Bad Request رد می‌کنه - یعنی *خودِ هشدار*
    # هیچ‌وقت به ادمین نمی‌رسید، دقیقاً وقتی که بیشتر از همیشه بهش نیاز بود (یک خطای واقعی).
    # html.escape این ریسک رو برای مسیرِ پیش‌فرض (context+error) کامل حذف می‌کنه.
    text = full_text if full_text is not None else (
        f"⚠️ <b>candle_engine error</b>\n"
        f"Context: <code>{html.escape(str(context))}</code>\n"
        f"Error: <code>{html.escape(str(error)[:500])}</code>\n"
        f"{_now_str()}"
    )
...
# 🔴 پیشگیرانه: همون الگوی ناامنِ os.getenv(key,"...") که باعث کرشِ Run #1522 در bot.py شد
# (توضیح کامل آنجا) - این متغیر فعلاً هیچ‌جا در ورک‌فلو پاس داده نمی‌شه، پس فعلاً کرش نمی‌کنه،
# ولی برای این‌که یک secret احتمالیِ بعدی همین بمبِ ساعتی رو دوباره فعال نکنه، با همون الگوی
# امنِ or هماهنگش می‌کنیم.
TWELVEDATA_DAILY_LIMIT = int(os.getenv("TWELVEDATA_DAILY_LIMIT") or "800")
...
def _load_closed_signal_ids() -> set:
    """🔴 اضافه شد: مجموعه‌ی شناسه‌ی همه‌ی معاملاتِ از قبل بسته‌شده (trade_history.json) - برای
    این‌که next_signal_id/_signal_id_in_use بتونن یک شناسه‌ی تازه رو هم در برابر معاملاتِ
    هنوز-باز و هم در برابر معاملاتِ از قبل بسته‌شده چک کنن (توضیحِ کامل در next_signal_id)."""
    history = read_json_resilient(
        TRADE_HISTORY_FILE, [], label="trade_history.json",
        on_corrupt=lambda label, msg: notify_admin(f"corrupt_{label}", Exception(msg)))
    return {h.get("signal_id") for h in history if h.get("signal_id")}


def _signal_id_in_use(signal_id: str, candle_states: Dict[str, Any], closed_ids: Optional[set] = None) -> bool:
    """چک می‌کنه یک signal_id همین الان روی یک معامله‌ی هنوز-باز دیگه استفاده شده یا نه، و
    (اگه closed_ids پاس داده شده باشه - نگاه کنید _load_closed_signal_ids) روی یک معامله‌ی
    از قبل بسته‌شده هم استفاده شده یا نه."""
    for s in candle_states.values():
        t = (s or {}).get("open_trade")
        if t and not t.get("closed") and t.get("signal_id") == signal_id:
            return True
    if closed_ids and signal_id in closed_ids:
        return True
    return False
...
def open_new_trade(signal: Dict[str, Any], symbol: str = None, tf_key: str = None, display: str = None,
    ...
    if preferred_signal_id:
        # 🔴 اضافه شد: قبلاً این چک فقط معاملاتِ هنوز-باز رو می‌دید - یعنی اگه preferred_signal_id
        # (مثلاً یک ALT-XXXXXX از منبع رله‌ی آلتکوین) دقیقاً با شناسه‌ی یک معامله‌ی از قبل
        # بسته‌شده یکی می‌بود، بدون هیچ هشدار/جایگزینی مستقیماً پذیرفته می‌شد - دقیقاً همون
        # کلاس‌باگِ «دو سیگنال با یک شناسه» که next_signal_id پایین‌تر (برای مسیرِ TC-XXXXXX)
        # از قبل جلوش گرفته شده بود.
        if candle_states is not None and _signal_id_in_use(preferred_signal_id, candle_states,
                                                             _load_closed_signal_ids()):
            logger.warning(f"preferred_signal_id {preferred_signal_id} already in use by an open or "
                            f"previously-closed trade - minting a fresh id instead to avoid a collision.")
        else:
            signal_id = preferred_signal_id
    if not signal_id:
        signal_id = next_signal_id(candle_states=candle_states)
    return {
        "signal_id": signal_id,
...
def check_open_trade(trade: Dict[str, Any], candle: Dict[str, Any], ema7_now: Optional[float] = None) -> List[Dict[str, Any]]:
    ...
    if side == "BUY":
        if candle["l"] <= sl:
            ...
            # 🔴 رفعِ باگِ محاسباتیِ رانر (توضیح کامل در docstring خودِ compute_final_r در
            # shared_risk_config.py): همین‌جا، در لحظه‌ای که واقعاً معلومه رانر با تریلینگ‌استاپ
            # روی چه قیمتی بسته شده، R واقعیِ اون رو محاسبه و روی trade ذخیره می‌کنیم - تا
            # compute_final_r دیگه مجبور نباشه فرض ثابت/تقریبیِ 6.0 رو استفاده کنه.
            if trade["close_type"] == "runner_stop":
                trade["runner_r"] = (sl - entry) / r
        ...
        if candle["h"] >= sl:
            trade["closed"] = True
            trade["close_type"] = stop_event_type()
            # 🔴 همون رفعِ باگِ محاسباتیِ رانر - نسخه‌ی SELL (نگاه کنید کامنتِ مشابه در شاخه‌ی BUY بالا).
            if trade["close_type"] == "runner_stop":
                trade["runner_r"] = (entry - sl) / r
            events.append({"type": trade["close_type"], "price": sl})
            return _persist(events)
        for target in RR_TARGETS:
            key = str(target)
...
def build_chart_from_hist(hist: List[Dict[str, Any]], title: str, trade: Optional[Dict[str, Any]] = None,
    ...
    try:
        ...
        # 🔴 رفعِ باگ: نسخه‌ی موازیِ این تابع در bot.py (ربات آلت‌کوین) از قبل بعد از savefig،
        # plt.close(fig) صدا می‌زد (دقیقاً مثل build_equity_chart همین فایل، چند ده خط بالاتر) -
        # ولی اینجا از قلم افتاده بود. matplotlib بدون close صریح، رفرنس هر Figure رو در
        # مدیریت داخلی خودش (pyplot state) نگه می‌داره - یعنی حافظه‌اش هیچ‌وقت آزاد نمی‌شه، نه
        # با پایان تابع نه با garbage collection معمولی. چون این تابع برای *هر* پیام چارت‌دار
        # (ورود + هر رویداد خروج: تارگت/استاپ/بریک‌ایون/رانر) در طول یک اجرای پیوسته‌ی ~۵
        # ساعت‌ونیم صدا زده می‌شه، این نشتی به‌مرور مصرف حافظه‌ی پروسه رو بالا می‌برد - در
        # بدترین حالت (روزهای پرسیگنال) می‌تونه به OOM-kill شدن پروسه توسط GitHub Actions
        # runner منجر بشه (که از بیرون دقیقاً مثل یک قطعی/تایم‌اوت بی‌دلیل دیده می‌شه).
        import matplotlib.pyplot as plt
        plt.close(fig)
        return CHART_PATH
    except Exception as e:
...
# 🔴 این دیکشنری از shared_risk_config.py import می‌شه (نه دیگه اینجا تعریف می‌شه) - قبلاً
# اینجا و توی bot.py دو کپیِ دستیِ جداگانه بود (با دو معنای متفاوت برای کلید: اینجا ضریب R،
# در bot.py شماره‌ی تارگت) که هیچ چیزی drift احتمالیِ بینشون رو تشخیص نمی‌داد. توضیح کامل
# در docstring خودِ TARGET_ACTION_LINE در shared_risk_config.py.
# ⚠️ نکته‌ی فرمت: نسخه‌ی مشترک دیگه پیشوند "\n\n" ندارد (متن خام، بدون whitespace چسبیده) -
# format_rr_exit_message پایین‌تر خودش این فاصله رو صریح مدیریت می‌کنه (رفعِ باگِ خط خالیِ
# اضافه که قبلاً اینجا وجود داشت).
...
def _fmt_price(value: float) -> str:
    """قیمت رو با تعداد رقم اعشار مناسبِ مقیاسش نشون می‌ده - الان صرفاً یک نام مستعارِ محلی
    برای shared_risk_config.format_signal_price است (رفعِ ناهماهنگیِ فرمت قیمت با bot.py:
    قبلاً اینجا با جداکننده‌ی هزارگان و bot.py بدون آن نمایش می‌داد - توضیح کامل در
    docstring خودِ format_signal_price در shared_risk_config.py). نام تابع (_fmt_price)
    عمداً همون قبلی نگه داشته شده تا نیازی به تغییر ده‌ها محل فراخوانی در همین فایل نباشه."""
    return _shared_format_signal_price(value)
...
def format_rr_exit_message(display: str, tf_label: str, trade: Dict[str, Any], event: Dict[str, Any]) -> str:
    ...
    # 🔴 رفعِ باگِ فاصله‌ی اضافه: قبلاً اینجا "Now ...\n" (یک \n) + action (که خودش با
    # "\n\n" شروع می‌شد) = سه \n پشت‌سرهم (دو خط خالی) قبل از خط "✂️ Close..." - نامتقارن
    # با فاصله‌ی یک‌خط‌خالیِ بقیه‌ی پیام. الان "Now ...\n\n" (دو \n صریح اینجا) + action
    # (که دیگه پیشوند \n\n نداره - نگاه کنید shared_risk_config.TARGET_ACTION_LINE) =
    # دقیقاً یک خط خالی، هماهنگ با همه‌جای دیگه‌ی پیام.
    return (
...
def process_symbol_timeframe(symbol: str, display: str, tf_key: str, tf_cfg: Dict[str, Any],
        ...
        for idx, k in enumerate(candles):
            ...
            if idx == last_idx and sig:
                ...
                if tf_key in DISABLED_AUTO_TIMEFRAMES:
                    # 🔴 این تایم‌فریم عمداً برای سیگنال‌های خودکارِ جدید خاموشه (بالاتر
                    # توضیح داده شد) - فقط رد می‌شیم، بدون باز کردن معامله. سیگنال‌های
                    # دستی/فوروارد‌شده روی همین تایم‌فریم اصلاً از این تابع رد نمی‌شن، پس
                    # دست‌نخورده می‌مونن.
                    continue
                ...
                # 🔴 باگ رفع‌شده: قبلاً candle_states اینجا پاس داده نمی‌شد - یعنی گاردِ ضدِ
                # تصادمِ next_signal_id (نگاه کنید next_signal_id بالاتر) برای این مسیر
                # (سیگنال‌های خودکارِ bootstrap) کاملاً غیرفعال بود، و یک شناسه‌ی تکراری
                # بی‌هیچ چکی صادر می‌شد.
    ...
    for idx, k in enumerate(new_candles):
        ...
        if sig and idx == last_new_idx:
            ...
            # 🔴 باگ رفع‌شده: قبلاً candle_states اینجا پاس داده نمی‌شد - همون باگِ گارد
            # غیرفعال که بالاتر (مسیر bootstrap) هم بود، اینجا هم بود؛ این خط اصلی‌ترین
            # مسیرِ باز شدنِ سیگنال‌های خودکارِ BTC/ETH است، پس اهمیتش از همه بیشتره.
            trade = None if tf_key in DISABLED_AUTO_TIMEFRAMES else open_new_trade(
                sig, symbol=symbol, tf_key=tf_key, display=display, candle_states=candle_states)
            # 🔴 توجه: اگه این تایم‌فریم خاموش باشه، اینجا trade=None می‌مونه (معامله‌ی جدید
            # باز نمی‌شه) - ولی force-close بالای همین بلوک (برای prev_trade/opp_trade، اگه
            # سیگنالِ مخالف باعث لازم‌شدنِ بستنِ یک معامله‌ی از قبل باز بشه) همچنان کامل اجرا
            # شده - یعنی هیچ معامله‌ای بی‌سروصدا باز نمی‌مونه، فقط معامله‌ی *جدید* باز نمی‌شه.
            if trade:
                apply_live_entry_price(symbol, sig, trade)
                state["open_trade"] = trade
                _send_entry(display, tf_key, tf_cfg, sig, trade, state["hist"])
...
def _force_close_trade(display, tf_key, tf_cfg, trade, candle, hist, symbol):
    ...
    exit_price = candle["c"]
    # 🔴 رفعِ باگِ واقعی («R نهاییِ بسته‌شدنِ اجباری همیشه اشتباه بود»): compute_final_r
    # (shared_risk_config.py) فرض می‌کنه باقی‌ماندهٔ پوزیشن همیشه دقیقاً روی یکی از سطوح
    # ثابت (Entry/Target1/Target2) بسته می‌شه - که فقط برای SL/بریک‌ایون/تارگتِ واقعی درسته
    # (چون working_sl واقعاً همونجا گذاشته شده). اینجا ولی باقی‌ماندهٔ پوزیشن روی قیمتِ
    # بازارِ همین لحظه (exit_price) بسته می‌شه، نه یکی از اون سطوح - یعنی بدون این فیکس،
    # compute_final_r همیشه یک عددِ ثابت/اشتباه برمی‌گردوند (مثلاً همیشه -1.0R اگه هنوز به
    # Target 1 نرسیده بود، صرف‌نظر از اینکه بسته‌شدنِ اجباری واقعاً با سود جزئی بود یا ضرر
    # جزئی) - و این عددِ اشتباه در پیام «⚠️ TRADE CLOSED»، trade_history.json، /results،
    # /history، گزارش روزانه/هفتگی، و equity chart تکرار می‌شد. اینجا R دقیقِ واقعی
    # محاسبه می‌شه: هر بخشی که واقعاً به یک تارگت رسیده با همون R دقیقِ تارگت بانک می‌شه،
    # و فقط بخشِ هنوز-باز (remaining_frac) با R واقعیِ همین قیمتِ بازار محاسبه می‌شه.
    sign = 1 if trade["side"] == "BUY" else -1
    realized_r_now = (exit_price - trade["entry"]) / trade["r"] * sign
    hit = trade["hit"]
    # 🔴 باگ مشابهِ همون باگِ compute_final_r (توضیح کامل در shared_risk_config.py) اینجا هم
    # وجود داشت: این تابع هم مستقیم hit.get("1")/("2")/("4")/("6") رو تک‌تک چک می‌کنه (نه از
    # طریق compute_final_r، چون منطقِ اینجا با بقیه فرق داره - باقی‌مانده روی قیمتِ بازار
    # بسته می‌شه نه یک سطحِ ثابت). این تابع برای بستنِ اجباریِ معاملاتِ رله‌شده/دستی
    # (tf_key="manual" - همون‌هایی که مستعدِ باگِ hit ناسازگارِ فوروارد‌شده‌ان، نگاه کنید
    # candle_engine.py::process_forwarded_results) هم صدا زده می‌شه - پس بدون همون
    # نرمالایزِ دفاعی، یک معامله‌ی رله‌شده که (به‌خاطر گم‌شدنِ پیامِ T1) hit["1"]=False ولی
    # hit["2"]/hit["4"] درست بودن، اینجا هم banked_r/banked_frac رو دستِ‌کم‌محاسبه می‌کرد.
    hit_levels = [t for t in RR_TARGETS if hit.get(str(t))]
    if hit_levels:
        max_hit = max(hit_levels)
        hit = {str(t): (hit.get(str(t)) or t <= max_hit) for t in RR_TARGETS}
    banked_r, banked_frac = 0.0, 0.0
    if hit.get("1"):
        banked_r += W1 * 1; banked_frac += W1
    if hit.get("2"):
        banked_r += W2 * 2; banked_frac += W2
    if hit.get("4"):
        banked_r += W3 * 4; banked_frac += W3
    if hit.get("6"):
        banked_r += W4 * 6; banked_frac += W4
    remaining_frac = max(0.0, 1.0 - banked_frac)
    trade["final_r_exact"] = banked_r + remaining_frac * realized_r_now
...
def process_manual_signals():
    ...
    for m in pending:
        try:
            ...
            # 🔴 رفعِ باگِ ریشه‌ایِ واقعی («سیگنال‌های رله‌شده‌ی آلتکوین گاهی هیچ‌وقت رهگیری
            # نمی‌شدند، بدون هیچ هشداری که قابل‌رفع باشه»): این چک را **فقط** برای سیگنال‌های
            # واقعاً دستی (نه رله‌شده) انجام می‌دیم. چرا: این چک برای زمانی طراحی شده که ادمین
            # دستی صبر کنه/تایپ کنه (می‌تونه دقیقه‌ها طول بکشه) و بعدش این معامله واقعاً با
            # قیمتِ زنده‌ی خودمون رهگیری بشه - پس منطقی بود که اگه قیمت از قبل رد شده، باز نشه.
            # اما سیگنال‌های رله‌شده‌ی خودکار (forwarded_tracking/silent - همیشه هر دو با هم
            # True از handle_altcoin_relay_post) اصلاً هیچ‌وقت با قیمتِ زنده‌ی خودمون رهگیری
            # نمی‌شن (نه REST نه WS، نگاه کنید کامنتِ handle_altcoin_relay_post) - نتیجه‌شون
            # ۱۰۰٪ از روی پیام‌های بعدیِ همون کانال (با همون هشتگ) گرفته می‌شه، نه از مقایسه‌ی
            # قیمت. یعنی این چک برایشان از همون اول بی‌ربط بود، فقط مضر: تاخیرِ چند-ثانیه‌ایِ
            # طبیعیِ رله (کپی در تلگرام + همگام‌سازی گیت بین دو ریپو، ~۲۲ ثانیه میانگین) روی
            # تایم‌فریم‌های کوتاه با استاپِ نزدیک (۱m/۵m که اکثرِ سیگنال‌های آلتکوین هستن) کاملاً
            # کافیه که قیمت واقعاً رد بشه - و همین دقیقاً باعث می‌شد این سیگنال‌ها هرگز باز نشن،
            # پس هیچ‌وقت هم چیزی برای تطبیق با نتیجه‌ی بعدی‌شون وجود نداشت («⚠️ Forwarded result
            # never matched a trade» - دقیقاً همون هشداری که دیده شد). حذفِ این چک برای این نوع
            # سیگنال هیچ ریسکِ محاسباتی نداره چون نتیجه‌ی نهایی‌شون هیچ‌وقت از رویِ همین قیمتِ
            # زنده مشتق نمی‌شه.
            if not (m.get("forwarded_tracking") or m.get("silent")):
                live_price = None
                try:
                ...
                if live_price and r > 0:
                    ...
                    if already_stopped or already_maxed:
                        ...
                        # 🔴 رفعِ باگِ واقعیِ «سیگنال‌های رد‌شده بی‌صدا گم می‌شدند»: کلیدِ throttle قبلاً
                        # فقط symbol:side بود (نه شناسه‌ی خودِ این سیگنالِ مشخص) - یعنی اگه در یک بازه‌ی
                        # ۱۵دقیقه‌ای (ADMIN_ALERT_COOLDOWN_SECONDS) دو سیگنالِ کاملاً متفاوت برای همون
                        # symbol+side رد می‌شدن (کاملاً محتمل روی نمادهای پرتحرک)، فقط هشدارِ اولی
                        # می‌رفت و دومی به‌خاطر cooldown کاملاً بی‌صدا سرکوب می‌شد - انگار آن سیگنالِ دوم
                        # هرگز وجود نداشته. چون هر آیتمِ manual_signals.json فقط دقیقاً یک‌بار پردازش
                        # می‌شه (بعد از خروج از "pending" دیگه هیچ‌وقت دوباره بررسی نمی‌شه)، اضافه‌کردنِ
                        # m['id'] (که برای هر سیگنال همیشه یکتاست) کاملاً امن است - نه خطرِ اسپم (هر id
                        # فقط یک‌بار به اینجا می‌رسه) نه خطرِ سرکوبِ اشتباهیِ یک سیگنالِ دیگر.
                        notify_admin(
                            f"manual_signal_rejected:{symbol}:{side}:{m['id']}",
            ...
            if not matched_tf_key:
                ...
                notify_admin(
                    f"manual_signal_no_tf:{symbol}:{side}:{m['id']}",  # 🔴 همون فیکسِ throttle - نگاه کنید کامنتِ manual_signal_rejected بالاتر
                    Exception(f"{symbol} {side} entry={_fmt_price(entry)} sl={_fmt_price(sl)} was NOT posted — "
            ...
            if dup_trade:
                ...
                logger.info(f"⏭️ Skipping manual/forwarded signal {symbol} {side} entry={_fmt_price(entry)} — "
                            f"{m['reject_reason']}")
                notify_admin(
                    f"manual_signal_duplicate:{symbol}:{side}:{m['id']}",  # 🔴 همون فیکسِ throttle - نگاه کنید کامنتِ manual_signal_rejected بالاتر
            ...
            # 🔴 رفعِ باگِ واقعیِ «پیامِ بسته‌شدنِ دستیِ ادمین (/admin_close_trade) روی سیگنال‌های
            # رله‌شده‌ی آلتکوین به هیچ پیامی ریپلای نمی‌کرد»: طبق درخواست صریح («هر پیام مربوط به
            # سیگنالِ اولیه‌ی رمزارز به سیگنالِ اولیه حتماً ریپلای شود»)، هر پیامی که بعداً از
            # سمتِ candle_engine.py برای این معامله ارسال بشه (مثلاً همون admin-close) باید به
            # پیامِ ورودیِ همون معامله ریپلای کنه. برای مسیرِ silent=True (پایین‌تر)، چون هیچ پیامِ
            # محلی‌ای پست نمی‌شه، trade["signal_message_id"] تا الان هیچ‌وقت ست نمی‌شد. الان اگه
            # subscription_bot.py قبلاً message_id همون کپیِ منتشرشده در کانال VIP را پاس داده
            # باشه (handle_altcoin_relay_post::queue_manual_signal(signal_message_id=...))، همینجا
            # مستقیم روی trade ست می‌شه - بدون نیاز به هیچ پستِ جدید. برای مسیرِ غیر-silent، این
            # مقدار معمولاً None است و _send_entry پایین‌تر آن را با شناسه‌ی واقعیِ پیامِ تازه‌پست‌شده
            # جایگزین می‌کند (رفتار قبلی دست‌نخورده می‌ماند).
            if m.get("signal_message_id"):
                trade["signal_message_id"] = m["signal_message_id"]
            ...
            notify_admin(f"manual_signal_item_crashed:{m.get('id')}", Exception(  # 🔴 همون فیکسِ throttle - نگاه کنید کامنتِ manual_signal_rejected بالاتر (اونجا فقط symbol:side بود، اینجا حتی نماد هم نبود - وضعش بدتر بود)
...
def process_forwarded_results():
    ...
    for ev in pending:
        try:
            ...
            if not best_trade:
                # 🔴 دفاعِ لایه‌ی دوم (defense-in-depth) در کنار فیکسِ ریشه‌ای در
                # subscription_bot.py::handle_channel_signal_post (که دیگه پیام‌های غیرِفوروارد
                # رو اصلاً صف نمی‌کنه): اگه این رویداد یک source_signal_id مشخص داره و اون
                # signal_id از قبل - از هر مسیری (رهگیریِ زنده، admin_override، یا یک تطبیقِ
                # قبلیِ همین صف) - در trade_history.json بسته ثبت شده، این رویداد صرفاً یک
                # اکوی بی‌ضرر از یک بسته‌شدنِ درستِ قبلیه، نه یک خطای واقعی. به‌جای اینکه بعد از
                # چند تلاش به‌عنوان «unmatched» با هشدارِ کاذب به ادمین گزارش بشه، همینجا ساکت
                # علامت می‌خوره - هیچ هشداری لازم نیست، چون نتیجه از قبل درست محاسبه شده.
                if source_sid:
                    history = read_json_resilient(TRADE_HISTORY_FILE, [], label="trade_history.json",
                                                   on_corrupt=lambda label, msg: notify_admin(f"corrupt_{label}", Exception(msg)))
                    if any(h.get("signal_id") == source_sid for h in history):
                        ev["status"] = "already_closed"
                        logger.info(f"Forwarded result for {source_sid} ignored - already closed in "
                                    f"trade_history.json (benign echo, not an error).")
                        continue
                ...
                if ev["attempts"] >= FORWARDED_RESULT_MAX_ATTEMPTS:
                    ...
                    # 🔴 همون فیکسِ throttle (نگاه کنید کامنتِ manual_signal_rejected در
                    # process_manual_and_forwarded_queues بالاتر در همین فایل): قبلاً کلید فقط
                    # {ticker} بود - یعنی اگه دو نتیجه‌ی تطبیق‌نیافته‌ی *متفاوت* برای همون تیکر
                    # (مثلاً یک استاپ روی ADA و بعداً یک تارگت‌هیت روی یک ADA دیگه) در یک بازه‌ی
                    # ۱۵دقیقه‌ای پیش می‌اومد، هشدارِ دومی بی‌صدا سرکوب می‌شد - دقیقاً همون سیگنالی
                    # که این‌بار واقعاً «گم» می‌شد. الان با source_sid (شناسه‌ی دقیقِ هشتگِ سیگنال،
                    # وقتی موجوده) یا queued_at (وقتی فرمت قدیمی بدون هشتگه) یکتا می‌شه.
                    unmatched_key = source_sid or f"{ticker}:{kind}:{ev.get('queued_at')}"
                    notify_admin(f"forwarded_result_unmatched:{unmatched_key}", Exception(text),
            ...
            if kind == "target_hit":
                ...
                if level is not None and str(level) in best_trade["hit"]:
                    # 🔴 باگ رفع‌شده («banked so far» منفی برای معاملاتی که واقعاً تا T2/T3
                    # رسیده بودن، مثلاً TAO/USD ALT-000133): قبلاً اینجا فقط دقیقاً همون
                    # سطحِ گزارش‌شده در این رویداد True می‌شد. اگه سطح پایین‌تر (مثلاً T1)
                    # هیچ‌وقت با موفقیت فوروارد/تطبیق نشده باشه (پیام گم شده، یا با تاخیر و
                    # هنوز در صفِ تطبیق مونده) ولی سطح بالاتر (T2/T3) برسه، hit["1"] برای
                    # همیشه False می‌موند - در حالی که رسیدنِ قیمت به یک تارگتِ بالاتر بدون
                    # عبور از سطوحِ پایین‌ترش از نظر منطقی اصلاً ممکن نیست. چون
                    # compute_final_r (در shared_risk_config.py) به‌ترتیب از hit["1"] شروع
                    # می‌کنه، این یعنی -1.0R (دقیقاً مثل خوردن کامل به حد ضرر) برمی‌گردوند،
                    # حتی وقتی معامله واقعاً تا T2/T3 پیش رفته بود - همون چیزی که در
                    # «Open right now» /admin_status دیده شد. الان دقیقاً مثل backfill ای که
                    # چند خط پایین‌تر برای کیندهای بسته‌شدن (breakeven/sl_after_t2/...) از
                    # قبل وجود داشت: رسیدن به یک سطح یعنی همه‌ی سطوحِ پایین‌ترش هم hit
                    # می‌شن، نه فقط همون یکی.
                    for t in RR_TARGETS:
                        if t <= level:
                            best_trade["hit"][str(t)] = True
                    changed = True
            ...
            ev["status"] = "error"
            # 🔴 همون فیکسِ throttle (نگاه کنید کامنتِ manual_signal_rejected بالاتر در همین
            # فایل): قبلاً این کلید کلاً ثابت بود ("forwarded_result_item_crashed" - بدون حتی
            # نماد!) - یعنی اگه دو رویدادِ کاملاً متفاوت (مثلاً یکی BTC یکی ETH) هر دو در یک
            # بازه‌ی ۱۵دقیقه‌ای کرش می‌کردن، فقط اولی گزارش می‌شد.
            crash_key = ev.get("source_signal_id") or f"{ev.get('ticker')}:{ev.get('kind')}:{ev.get('queued_at')}"
            notify_admin(f"forwarded_result_item_crashed:{crash_key}", Exception(
    ...
    for r in pending:
        ...
        if not t:
            ...
            not_found_text = (
                ...
                f"Requested key: <code>{html.escape(str(key))}</code>\n\n"
...
def run_scan_cycle(latest_price_ts: Optional[Dict[str, float]] = None, price_lock=None):
    ...
    try:
        check_stale_open_trades(state, candle_states)
    except Exception as e:
...
# لزوماً معامله‌ی واقعی هنوز واقعاً باز باشه.
#
# 🔴 رفعِ باگِ واقعیِ «همین یک هشدار مدام (هر چند دقیقه) دوباره فرستاده می‌شه، حتی بعد از
# کلیک روی دکمه‌ها»: ریشه‌ش این بود که cooldown قبلی (_LAST_ADMIN_ALERT در notify_admin) فقط
# در حافظه‌ی پروسه بود - نه در candle_state.json - پس با هر ری‌استارتِ این اسکریپت (که طبق
# self-chain هر ~۵ ساعت و ۲۰ دقیقه طبیعتاً اتفاق می‌افته) کاملاً صفر می‌شد، و حتی در یک
# پروسه‌ی واحد هم فقط ۱۵ دقیقه (ADMIN_ALERT_COOLDOWN_SECONDS) دوام داشت - یعنی برای یک
# معامله‌ای که واقعاً گیر کرده (و تا وقتی ادمین دستی حلش نکنه، گیر می‌مونه)، این هشدار
# می‌تونست ده‌ها بار در روز تکرار بشه. الان زمانِ آخرین هشدار مستقیم روی خودِ trade ذخیره
# می‌شه (t["_stale_alert_sent_at_ms"]) که در candle_state.json پرسیست/گیت‌سینک می‌شه - یعنی
# «فقط یک‌بار» واقعاً یک‌بار می‌مونه، حتی بعد از ری‌استارت؛ فقط اگه بعد از یک یادآوریِ
# طولانی‌تر (STALE_ALERT_REPEAT_HOURS) هنوز حل نشده باشه، یک یادآوریِ دوم می‌رسه - نه هر
# چند دقیقه.
#
# 🔴 اضافه شد (ریشه‌ای‌تر از فیکسِ بالا): برای BTC/USD و ETH/USD - تنها نمادهایی که همیشه یک
# منبع قیمتِ زنده و قابل‌اعتماد داریم (WS_SYMBOL_MAP)، صرف‌نظر از اینکه این معامله
# forwarded_tracking باشه یا نه - قبل از هر هشداری، اول با قیمتِ واقعیِ همین‌الان چک می‌کنیم.
# چرا این با احتیاطِ طراحیِ اصلی («هیچ‌وقت خودکار نبند، چون نتیجه‌ی حدسی از هیچی بدتره») در
# تضاد نیست: اونجا نگرانی حدس‌زدنِ نتیجه بدون داده‌ی واقعی بود؛ اینجا قیمتِ واقعیِ زنده داریم -
# این حدس نیست، دقیقاً همون داده‌ای که اگه این معامله هیچ‌وقت forwarded_tracking نشده بود،
# با WS بلادرنگ چک می‌شد. این دقیقاً ریشه‌ی چیزیه که خودِ متنِ این هشدار مدت‌ها پیش تشخیص داده
# بود («Likely stuck because its close event was missed/unmatched»): یک معامله‌ی BTC/ETH که
# به‌اشتباه (مثلاً یک ادمین پیامش رو در کانال فوروارد کرده) forwarded_tracking شده،
# check_ws_open_trades همیشه ردش می‌کنه (چون forwarded_tracking=True)، و چون سیستم رله‌ی
# نتیجه‌ی فوروارد‌شده فقط برای آلتکوین‌هاست (نه BTC/ETH)، هیچ پیامِ نتیجه‌ای هم هیچ‌وقت
# نمی‌رسه که ببندش - یعنی برای همیشه گیر می‌کرد. نتیجه‌ی این چک، دو حالت دارد: یا واقعاً به
# استاپ/تارگتی رسیده (پس همین‌جا با قیمتِ واقعی بسته می‌شه - دقیق، نه حدسی)، یا هنوز واقعاً
# به هیچ‌کدوم نرسیده (پس واقعاً هنوز باز است، نه گیرکرده - و هشدار اصلاً لازم نیست، چون همین
# چک خودش این ابهام رو برطرف کرده). در هر دو حالت نیازی به هشدار به ادمین نیست؛ هشدار فقط
# برای نمادهایی می‌مونه که این منبع قیمتِ زنده‌ی قابل‌اعتماد رو نداریم (مثلاً آلتکوین‌های
# رله‌شده) - اونجا واقعاً فقط تصمیمِ دستی معنا داره.
STALE_OPEN_TRADE_HOURS = 48
STALE_ALERT_REPEAT_HOURS = 24  # بعد از هشدارِ اول، حداکثر هر ۲۴ ساعت یک یادآوری - نه هر چند دقیقه


def check_stale_open_trades(state: Dict[str, Any], candle_states: Dict[str, Any]) -> None:
    now_ms = int(time.time() * 1000)
    ...
    for key, s in candle_states.items():
        ...
        if symbol in WS_SYMBOL_MAP:
            # --- مسیر خودترمیم‌گر با قیمت واقعی - توضیح کامل بالای این تابع ---
            try:
                live_price = fetch_live_price(symbol)
            except Exception as e:
                logger.warning(f"stale-trade live-price fallback failed for {key}: {e}")
                live_price = None
            if live_price:
                events = check_open_trade_live(t, live_price)
                if events:
                    display = t.get("display") or WATCHLIST_SYMBOLS.get(symbol, symbol)
                    tf_key = t.get("tf_key") or "manual"
                    tf_cfg = t.get("manual_tf_cfg") or TIMEFRAMES.get(tf_key, MANUAL_TF_CFG)
                    for ev in events:
                        _send_exit(display, tf_key, tf_cfg, t, ev, s.get("hist"), symbol, live_price=live_price)
                    state_changed = True
                    logger.info(f"✅ Self-healed stuck trade {t.get('signal_id', key)} using live price "
                                f"(was likely stuck due to forwarded_tracking with no matching forwarded result)")
                # چه بسته شد (رویداد داشت) چه واقعاً هنوز به هیچ سطحی نرسیده (رویدادی نداشت) -
                # در هر دو حالت دیگه ابهامی نیست، پس نیازی به هشدار به ادمین نیست.
                continue
            # قیمتِ زنده هم نتونستیم بگیریم - این خیلی نادره (یعنی حتی fetch_live_price هم شکست
            # خورده)؛ فقط در این حالتِ نادر به مسیر هشدارِ زیر (persisted one-shot) سقوط می‌کنیم.

        # --- از اینجا به بعد: نمادهایی که منبع قیمتِ زنده‌ی قابل‌اعتماد نداریم (مثلاً
        # آلتکوین‌های رله‌شده) - یا آن مورد نادرِ fetch_live_price ناموفق - اونجا واقعاً فقط
        # هشدار/تصمیمِ دستی معنا داره. ---

        last_alert_ms = t.get("_stale_alert_sent_at_ms")
        if last_alert_ms and (now_ms - last_alert_ms) < STALE_ALERT_REPEAT_HOURS * 3600 * 1000:
            continue  # قبلاً اطلاع دادیم؛ موعدِ یادآوریِ بعدی هنوز نرسیده - ادمین رو اسپم نکن
        t["_stale_alert_sent_at_ms"] = now_ms
        state_changed = True
