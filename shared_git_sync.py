...
def pull_latest_readonly(repo_dir: str, stash_message: str = "pull_latest_readonly: temporary stash",
    ...
    candle_state.json را می‌نویسد و پوش می‌کند، هر ۴۵ ثانیه) قبلش موفق پوش کرده باشد. حالا
    /results و /pnl قبل از خوندن، یک pull سبک و فوری انجام می‌دن، پس همیشه آخرین نسخه‌ی
    موجود روی ریموت را نشون می‌دن.

    🔴 رفعِ باگِ دوم («could not pull the latest data» که به‌طور مکرر برای کاربر دیده شد):
    نسخه‌ی قبلیِ این تابع، تغییراتِ محلیِ کامیت‌نشده را قبل از pull، commit می‌کرد (تا درخت
    برای rebase تمیز بشه). مشکل: چون این تابع خیلی مکرر صدا زده می‌شه (هر بار هرکسی /results
    یا مشابهش رو می‌زنه)، هر بار که چیزی کثیف بود یک کامیتِ *دائمیِ* تازه می‌ساخت - و چون این
    کامیت‌های ریز هیچ‌وقت همون‌جا push نمی‌شدن (فقط چرخه‌ی دوره‌ای بعدی push می‌کرد)، ممکن بود
    چند کامیتِ محلیِ push‌نشده روی هم انباشته بشن، هرکدوم باید در برابر یک remote HEAD که
    توسط candle_engine.py هر ۴۵ ثانیه جلو می‌ره rebase بشن - یعنی هرچی این تابع مکرر‌تر صدا
    زده می‌شد، احتمالِ برخوردِ واقعیِ محتوایی (نه فقط درختِ کثیف) بین این زنجیره‌ی کامیت‌های
    محلی و remoteِ درحال‌حرکت بیشتر می‌شد - دقیقاً همون تکرارشدنِ این هشدار که گزارش شد.
    راه‌حلِ درست‌تر: به‌جای commit (که تاریخچه‌ی گیت رو دائمی درگیر می‌کنه)، از `git stash`
    استفاده می‌کنیم - تغییراتِ محلی موقتاً به‌طور کامل کنار گذاشته می‌شن (نه کامیت)، pull روی
    یک درختِ کاملاً تمیز انجام می‌شه، و بلافاصله بعد تغییراتِ محلی برمی‌گردن (stash pop). این
    تابع هیچ‌وقت اثری در تاریخچه‌ی گیت باقی نمی‌ذاره و هیچ کامیتی برای rebase-شدن در برابر
    remoteِ درحال‌حرکت نمی‌سازه - ریشه‌ی تکرارشدن کاملاً حذف می‌شه.

    best-effort و بی‌خطر: اگه pull شکست بخوره (تعارضِ محتواییِ واقعی یا مشکل شبکه)، rebase را
    abort می‌کنه و False برمی‌گردونه؛ در هر صورت (موفق یا ناموفق) تلاش می‌کنه stash رو برگردونه
    تا تغییراتِ محلی گم نشن. اگه حتی stash pop هم (به‌ندرت، فقط در یک تعارضِ واقعیِ محتوایی)
    شکست بخوره، stash دست‌نخورده در لیست می‌مونه (چیزی گم نمی‌شه، فقط نیاز به بررسیِ دستی)."""
    def run(args):
        return _run_git(args, cwd=repo_dir)
    ...
    if stashed:
        ...
        if pop_res.returncode != 0:
            # 🔴 حیاتی: اگه دقیقاً همون خط از همون فایل هم توسط pull تازه تغییر کرده باشه هم
            # توسط تغییرِ محلیِ استش‌شده (که تست مستقیم نشون داد واقعاً ممکنه - مثلاً هر دو
            # پروسه به manual_signals.json/admin_close_requests.json می‌نویسن)، stash pop
            # می‌تونه با یک تعارضِ واقعی شکست بخوره و فایل را با نشانه‌های تعارضِ گیت
            # (<<<<<<<) روی دیسک رها کنه - یعنی از خودِ باگِ اولیه هم بدتر (یک JSON نامعتبر
            # به‌جای یک خطای تمیز). چون اولویتِ این تابع «همیشه یک درختِ کاریِ سالم و
            # قابل‌خواندن» است، نه «هیچ‌وقت هیچ تغییرِ محلی‌ای گم نشه»، درختِ کاری را به حالتِ
            # تمیزِ بلافاصله-بعد-از-pull برمی‌گردونیم. تغییرِ محلیِ کنارگذاشته‌شده گم نمی‌شه -
            # خودِ stash entry دست‌نخورده در لیست می‌مونه (stash pop با تعارض، entry رو drop
            # نمی‌کنه) و با `git stash show -p` بعداً دستی قابل‌بازیابیه.
