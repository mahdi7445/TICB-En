...
JSON_ARRAY_MERGE_BASENAMES = {"trade_history.json", "forwarded_results_queue.json",
                               ...
                               "relay_seen_ids.json", "used_tx.json", "pending_manual_fixes.json"}

# 🔴 اضافه شد: api_quota_state.json یک آرایه نیست (یک دیکشنریِ تختِ {date, count, warned_80}
# است)، پس union معنایی بالا اصلاً براش صدق نمی‌کنه - استراتژیِ جداگانه‌اش
# (_merge_counter_state، چند خط پایین‌تر) delta هر دو طرف رو نسبت به base جمع می‌زنه، نه
# صرفاً یکی رو نگه می‌داره.
COUNTER_MERGE_BASENAMES = {"api_quota_state.json"}

# 🔴 اضافه شد (بعد از دیدنِ همین الگوی دقیق دوبار پشتِ سرِ هم): candle_state.json هم یک
# دیکشنری‌ست، نه آرایه، پس نه union بالا و نه counter-merge براش صدق می‌کنه - ولی برخلافِ
# قبل، دیگه صرفاً «دست‌نزدن»ش رو انتخاب نکردم. هر ورودیِ candle_signals["SYMBOL|TF"] دقیقاً
# ۱۹ فیلدِ شناخته‌شده داره (new_candle_state در candle_engine.py، بررسیِ کاملِ کد، نه حدس):
# ۹ تاشون (CANDLE_STATE_SAFE_FIELDS پایین‌تر) کش‌های عددیِ صرفاً-محاسباتی‌ان که هر چرخه از
# نو، از رویِ hist بازمحاسبه می‌شن (ema7/ema25/atr/...) - اختلافِ جزئیِ بینِ دو محاسبه‌ی
# هم‌زمان روشون تاثیرِ عملیاتی نداره (میانگینِ نماییِ smoothing، خودش رو خود-اصلاح می‌کنه).
# ۱۰ تای دیگه (hist و از همه مهم‌تر open_trade، بعلاوه‌ی وضعیتِ تشخیصِ سیگنال مثل
# trend_prev/bull_used_this_trend/bar_index) اصلاً به این merge سفید-لیست راه داده نمی‌شن.
# _merge_candle_state (پایین‌تر) این تفکیک رو *برای هر SYMBOL|TF جداگانه* enforce می‌کنه:
# اگه غیر از فیلدهای امن، حتی یک فیلدِ دیگه (خصوصاً open_trade یا hist) هم بینِ دو طرف فرق
# داشته باشه، کل merge رد می‌شه (None برمی‌گرده) و رفتارِ امنِ فعلی (hard-reset، که فقط
# حداکثر یک چرخه از همین کش‌های ارزون رو از دست می‌ده) جایگزینش می‌شه - دقیقاً همون فلسفه‌ی
# همه‌جای این فایل: merge خودکار فقط وقتی انجام می‌شه که ریاضیاتاً/معنایی ثابت‌شده امن باشه؛
# در غیر این صورت هیچ کاری نمی‌کنه، نه یک حدسِ خطرناک.
CANDLE_STATE_BASENAMES = {"candle_state.json"}
CANDLE_STATE_SAFE_FIELDS = {"ema7", "ema25", "atr", "tr_buffer",
                             "plus_dm14", "minus_dm14", "dm_buffer", "adx", "dx_buffer"}
...
#
# 🔴 اضافه شد (طبق تأیید صریح برای دو فایلِ هم‌الگو، بعد از بررسیِ کاملِ هرکدوم):
#   - used_tx.json: آرایه‌ی ساده‌ی شناسه‌ی تراکنش‌های USDT از قبل تاییدشده (try_confirm_payment
#     - subscription_bot.py) - عیناً همون الگو: فقط append، فقط چکِ عضویت (tx_id in used_tx)،
#     هیچ رکوردی هیچ‌وقت حذف/ویرایش نمی‌شه، فقط با used_tx[-800:] محدود می‌شه. اینجا خطرِ
#     رفتارِ *قبلی* (hard-reset دور ریختنِ یک used_tx.append محلی‌ِ pushنشده) از همه‌ی
#     فایل‌های بالا جدی‌تر بود: یعنی یک تراکنشِ همین الان تاییدشده، بعد از هارد‌ریست دیگه
#     "استفاده‌شده" به‌حساب نمی‌اومد - اگه همون تراکنش در یک چرخه‌ی بعدی باز هم در نتیجه‌ی
#     fetch_wallet_transfers برگرده (کاملاً محتمل، چون این fetch یک بازه‌ی زمانیِ گذشته رو هر
#     بار دوباره می‌خونه)، می‌تونست باعث تاییدِ *دوباره*‌ی همون پرداخت و اعتبارِ اشتراکِ
#     تکراری/رایگان بشه. الان با union دیگه هیچ used_tx.append ثبت‌شده‌ای، حتی موقتاً، گم
#     نمی‌شه.
#   - pending_manual_fixes.json: آرایه‌ی درخواست‌های نیمه‌کاره‌ی /admin_manual_signal که با
#     دکمه‌ی تایم‌فریم تکمیل می‌شن (_queue_pending_manual_fix). همون‌طور با used_tx.json/
#     relay_seen_ids.json - فقط append، فقط خوانده می‌شه (هیچ‌جا حذف/ویرایش نمی‌شه)، با
#     fixes[-200:] محدود می‌شه - پس union اینجا هم کاملاً امنه.
...
def _merge_counter_state(remote_text: str, local_text: str, base_text: Optional[str]):
    """🔴 اضافه شد طبق درخواست صریح («تعارضاتِ پیش‌آمده رو اصولی حل کن»): برای فایل‌های
    شمارنده‌ی روزانه مثل api_quota_state.json ({"date", "count", "warned_80"}).

    برخلافِ فایل‌های آرایه‌ای بالا، اینجا union معنی نداره - این یک دیکشنریِ تخت با یک عددِ
    در حالِ افزایشه، نه یک لیستِ رکورد. تعارض روی این فایل یعنی دو اجرای هم‌پوشان، هرکدوم
    مستقل count رو increment کرده و commit کرده - اگه فقط یک طرف (مثلاً هرکدوم که در rebase
    برنده بشه) نگه داشته بشه، افزایشِ طرفِ دیگه برای همیشه گم می‌شه، یعنی شمارشِ واقعیِ
    مصرفِ API کمتر از واقعیت ثبت می‌شه - دقیقاً وقتی این خطرناکه که این فایل باید هشدارِ
    نزدیک‌شدن به سقفِ روزانه‌ی Twelve Data رو بده: با شمارشِ کمتر از واقعیت، ممکنه هشدار
    اصلاً به‌موقع فرستاده نشه و سهمیه بی‌خبر ته بکشه.

    منطق: هر دو طرف رو نسبت به نسخه‌ی مشترکِ base (مرحله‌ی :1: تعارض) delta می‌گیره و هر دو
    delta رو جمع می‌زنه (base_count + delta_remote + delta_local) - یعنی *هر دو* دسته
    افزایشِ واقعی حساب می‌شه، نه فقط یکی. اگه base در دسترس نباشه یا date مطابقت نداشته
    باشه، به‌جای حدسِ نادرست، محافظه‌کارانه min(remote,local) رو به‌عنوان base فرض می‌کنه -
    که یعنی نتیجه‌ی نهایی هیچ‌وقت کمتر از remote+local واقعی نمی‌شه (در بدترین حالت کمی
    دست‌بالا می‌گیره، که برای یک هشدارِ سهمیه، طرفِ امنِ خطاست، نه طرفِ خطرناکش).
    warned_80 هم OR می‌شه (یک پرچمِ یک‌طرفه‌ست - یک‌بار True شد، دیگه نباید False بشه).

    اگه date دو طرف فرق کنه (دقیقاً روی مرزِ نیمه‌شبِ UTC تعارض افتاده)، شمارشِ روزِ قدیمی‌تر
    دیگه بی‌معنیه - فقط جدیدترین روز نگه داشته می‌شه، بدون جمع (چون طرفِ روزِ قدیمی داشته
    برای یک روزِ کاملاً متفاوت می‌شمرده).

    None برمی‌گردونه اگه هرکدوم JSON/دیکشنری نباشه یا شکلِ غیرمنتظره‌ای داشته باشه - یعنی
    merge معنایی امن نیست، مثل بقیه‌ی این توابع."""
    allowed_fields = {"date", "count", "warned_80"}
    try:
        remote = json.loads(remote_text)
        local = json.loads(local_text)
    except Exception:
        return None
    if not (isinstance(remote, dict) and isinstance(local, dict)):
        return None
    if (set(remote.keys()) - allowed_fields) or (set(local.keys()) - allowed_fields):
        return None
    if not (isinstance(remote.get("count"), int) and isinstance(local.get("count"), int)
            and isinstance(remote.get("date"), str) and isinstance(local.get("date"), str)):
        return None
    if remote["date"] != local["date"]:
        newer = remote if remote["date"] > local["date"] else local
        return {"date": newer["date"], "count": max(int(newer.get("count", 0)), 0),
                "warned_80": bool(newer.get("warned_80"))}
    base_count = None
    if base_text:
        try:
            base = json.loads(base_text)
            if (isinstance(base, dict) and base.get("date") == remote["date"]
                    and isinstance(base.get("count"), int)):
                base_count = base["count"]
        except Exception:
            base_count = None
    if base_count is None:
        base_count = min(remote["count"], local["count"])
    merged_count = remote["count"] + local["count"] - base_count
    if merged_count < max(remote["count"], local["count"]):
        merged_count = remote["count"] + local["count"]
    return {"date": remote["date"], "count": merged_count,
            "warned_80": bool(remote.get("warned_80")) or bool(local.get("warned_80"))}


def _merge_candle_state(remote_text: str, local_text: str):
    """🔴 توضیحِ کاملِ چرایی/شرطِ ایمنی در تعریفِ CANDLE_STATE_SAFE_FIELDS بالاتره.

    برای هر کلیدِ candle_signals («SYMBOL|TF»)، اگه دقیقاً یکسانه یا فقط یک طرف داره، همون
    نگه داشته می‌شه. اگه هر دو طرف دارنش و فرق دارن، فقط وقتی امنه که *همه‌ی* فیلدهایی که
    فرق دارن عضوِ CANDLE_STATE_SAFE_FIELDS باشن - یعنی open_trade، hist، و بقیه‌ی وضعیتِ
    تشخیصِ سیگنال حتماً بینِ دو طرف کاملاً یکسان باشن. اگه حتی یک کلید این شرط رو نداشته
    باشه، کلِ merge (نه فقط همون کلید) رد می‌شه و None برمی‌گرده - چون این‌جا امکانِ «فقط
    یک کلید رو resolve کن، بقیه رو به حالتِ تعارض بذار» وجود نداره؛ فراخوان (مثلِ همه‌جای
    این فایل) باید abort کنه و به رفتارِ امنِ retry/hard-reset برگرده.
    None برمی‌گردونه اگه هرکدوم JSON/دیکشنریِ معتبر نباشه یا شکلِ سطحِ بالا غیرمنتظره باشه."""
    try:
        remote = json.loads(remote_text)
        local = json.loads(local_text)
    except Exception:
    ...
    if not (isinstance(remote, dict) and isinstance(local, dict)):
        return None
    if set(remote.keys()) != {"candle_signals"} or set(local.keys()) != {"candle_signals"}:
        return None  # شکلِ سطحِ بالای ناشناخته - محافظه‌کارانه دست نمی‌زنیم
    remote_sig, local_sig = remote["candle_signals"], local["candle_signals"]
    if not (isinstance(remote_sig, dict) and isinstance(local_sig, dict)):
        return None
    merged_sig = {}
    for key in set(remote_sig.keys()) | set(local_sig.keys()):
        r, l = remote_sig.get(key), local_sig.get(key)
        if r is None:
            merged_sig[key] = l
            continue
        if l is None:
        ...
        if not (isinstance(r, dict) and isinstance(l, dict)):
            return None
        differing = {f for f in (set(r.keys()) | set(l.keys())) if r.get(f) != l.get(f)}
        if differing - CANDLE_STATE_SAFE_FIELDS:
            return None  # چیزی غیر از کش‌های عددیِ امن فرق کرده (مثلاً open_trade/hist)
        merged = dict(r)
        for f in differing:
            if f in l:
                merged[f] = l[f]
        merged_sig[key] = merged
    return {"candle_signals": merged_sig}


def _try_resolve_conflicts_semantically(repo_dir: str, run) -> bool:
    """بعد از یک `git pull --rebase` شکست‌خورده (و در حال conflict)، سعی می‌کنه هر فایل
    تعارض‌دار رو با استراتژیِ مخصوصِ شکلِ همون فایل resolve کنه: آرایه‌ها با union معنایی
    (_dedup_json_array_union)، شمارنده‌های روزانه با جمع‌زدنِ delta (_merge_counter_state)،
    candle_state.json با merge سفید-لیستِ فیلدهای صرفاً-محاسباتی (_merge_candle_state).
    اگه هر فایل تعارض‌دارِ دیگه‌ای هم باشه (در هیچ‌کدوم از این سه لیست نه)، یا شکلش با
    استراتژیِ متناظرش نخونه، کاری نمی‌کنه و False برمی‌گردونه - یعنی فراخوان باید مثل قبل
    abort کنه. فقط وقتی همه‌ی تعارض‌ها resolve و add بشن، و `git rebase --continue` موفق
    بشه، True برمی‌گردونه.
    🔴 قبلاً این تابع فقط آرایه‌ها رو پوشش می‌داد (`_try_resolve_conflicts_as_json_arrays`)."""
    ...
    for path in conflicted:
        ...
        if (basename not in JSON_ARRAY_MERGE_BASENAMES and basename not in COUNTER_MERGE_BASENAMES
                and basename not in CANDLE_STATE_BASENAMES):
            return False  # فایل ناشناخته/شکل‌نامشخص در تعارض - merge معنایی امن نیست

    for path in conflicted:
        basename = os.path.basename(path)
        remote_res = run(["git", "show", f":2:{path}"])
        local_res = run(["git", "show", f":3:{path}"])
        if remote_res.returncode != 0 or local_res.returncode != 0:
            return False
        if basename in COUNTER_MERGE_BASENAMES:
            base_res = run(["git", "show", f":1:{path}"])
            merged = _merge_counter_state(remote_res.stdout, local_res.stdout,
                                           base_res.stdout if base_res.returncode == 0 else None)
        elif basename in CANDLE_STATE_BASENAMES:
            merged = _merge_candle_state(remote_res.stdout, local_res.stdout)
        else:
            merged = _dedup_json_array_union(remote_res.stdout, local_res.stdout, basename=basename)
        if merged is None:
            return False
...
def _conflict_summary(repo_dir: str, run, max_files: int = 3, max_chars_per_file: int = 350) -> str:
    ...
    for path in conflicted[:max_files]:
        # 🔴 باگِ برچسبِ جابه‌جا رفع شد: طبق تستِ عملیِ مستقیمِ رفتارِ گیت (نه فرض/حدس)، در
        # یک تعارضِ `git pull --rebase`، مرحله‌ی :2: («ours» از دیدِ خودِ گیت) همیشه سمتِ
        # upstream/remote است و مرحله‌ی :3: («theirs») همیشه commit محلیِ در حالِ replay -
        # دقیقاً برعکسِ نامگذاریِ intuitive و برعکسِ چیزی که این خط قبلاً فرض کرده بود (کامنتِ
        # قبلی می‌گفت :2:=محلی/:3:=remote). نتیجه‌ی این اشتباه: از وقتی این تابع اضافه شد، هر
        # پیامِ «Conflicted file(s)» که برای مهدی فرستاده می‌شد، برچسبِ remote/local روی
        # محتوای واقعی برعکس بود (خودِ محتوا/diff درست بود، فقط این‌که کدوم طرف کدومه، غلط
        # نشون داده می‌شد) - نه یک ریسکِ داده‌ای (merge واقعی، یعنی _dedup_json_array_union
        # چند خط پایین‌تر، از اول این دو مرحله رو درست map کرده بود)، فقط گمراه‌کننده برای
        # خوندنِ دستیِ همین پیامِ تشخیصی.
        remote_res = run(["git", "show", f":2:{path}"])
        local_res = run(["git", "show", f":3:{path}"])
        lines.append(f"— {path} —")
        if remote_res.returncode != 0 or local_res.returncode != 0:
        ...
        diff = list(difflib.unified_diff(
            remote_res.stdout.splitlines(), local_res.stdout.splitlines(),
...
def sync_data_dir(
    ...
    for attempt in range(1, max_retries + 1):
        ...
        if pull.returncode != 0:
            if _try_resolve_conflicts_semantically(repo_dir, run):
