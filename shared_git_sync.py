...
# صرفاً یکی رو نگه می‌داره. candle_state.json عمداً اینجا نیست: برخلافِ فایل‌های بالا، هر
# ورودیِ آن (candle_signals["SYMBOL|TF"]) هم شاملِ کش‌های عددیِ ارزان-به-بازمحاسبه (ema7/
# ema25/atr/...) و هم شاملِ وضعیتِ حیاتیِ تشخیصِ سیگنال (trend_prev/bull_used_this_trend/
# bar_index/...) و open_trade در یک آبجکتِ واحد است - انتخابِ خودکارِ «هر طرف» بینِ این دو
# دسته، بدون تفکیکِ درست، دقیقاً می‌تونه باعثِ همون باگِ سیگنالِ تکراری/جاافتاده بشه که این
# مکالمه صرفِ پیدا/رفع‌کردنش شده؛ برای همچین فایلی، حل‌شدنِ ایمن (با hard-reset فعلی، که
# محدود به گم‌شدنِ حداکثر یک چرخه از کش‌های ارزان‌قیمته) از یک merge خودکارِ پرریسک بهتره.
...
def _try_resolve_conflicts_semantically(repo_dir: str, run) -> bool:
    ...
    اگه هر فایل تعارض‌دارِ دیگه‌ای هم باشه (در هیچ‌کدوم از این دو لیست نه)، یا شکلش با
