...
JSON_ARRAY_MERGE_BASENAMES = {"trade_history.json", "forwarded_results_queue.json", "admin_close_requests.json"}

# 🔴 رفعِ باگِ واقعیِ «هشدار دروغینِ stuck/not_found برای معامله‌ای که کاملاً درست بسته شده»:
#
# ددوپ قبلی صرفاً روی برابریِ کاملِ JSON بود - که برای trade_history.json (رکوردهای واقعاً
# write-once، هیچ‌وقت بعد از ساخته‌شدن edit نمی‌شن) کاملاً درسته. اما برای این دو فایلِ دیگه
# غلطه: candle_engine.py میدانِ status (و برای صف نتایج فوروارد، attempts هم) رو روی *همون*
# رکوردِ از قبل موجود عوض می‌کنه (pending -> applied/not_found/unmatched/error) - نه این‌که
# یک رکورد جدید append کنه (process_forwarded_results/process_admin_close_requests در
# candle_engine.py). چون هر دو اسکریپت هر GIT_COMMIT_EVERY_SECONDS=۴۵ ثانیه commit می‌کنن،
