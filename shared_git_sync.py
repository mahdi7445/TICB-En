...
def pull_latest_readonly(repo_dir: str, commit_message: str = "auto-commit pending local changes before read-only sync [skip ci]",
                          max_retries: int = MAX_SYNC_RETRIES,
    ...
    قبل از خوندن، یک pull سبک و فوری انجام می‌دن، پس همیشه آخرین نسخه‌ی موجود روی ریموت را
    نشون می‌دن - نه نسخه‌ای که تصادفاً آخرین بار چرخه‌ی دوره‌ای به‌روزش کرده.

    🔴 رفعِ باگِ واقعیِ دومی که در همین مسیر پیدا شد («⚠️ could not pull the latest data»
    که واقعاً برای کاربر دیده شد): این تابع تا قبل از این خط، فرض می‌کرد working directory
    همیشه تمیز است (بدون تغییرِ محلیِ کامیت‌نشده) - چون فقط بین دو کامیتِ دوره‌ای صدا زده
    می‌شه. ولی subscription_bot.py هر بار کاربری کاری انجام بده (پرداخت تأیید بشه، مشترکِ
    جدید اضافه بشه، عکسِ سود ثبت بشه، ...) فوراً با atomic_write_json مستقیم روی دیسک
    می‌نویسه - نه فقط هر ۱۲۰ ثانیه؛ یعنی هر لحظه بینِ این نوشتنِ فوری و کامیتِ دوره‌ای بعدی،
    working directory «کثیف» (تغییرِ محلیِ کامیت‌نشده) است. وقتی pull_latest_readonly دقیقاً
    در همین بازه صدا زده می‌شه، `git pull --rebase` روی یک درختِ کثیف بلافاصله رد می‌شه (نه
    یک تعارضِ واقعیِ محتوایی، فقط امتناعِ گیت از rebase روی تغییرِ ثبت‌نشده) - و چون این می‌تونه
    هر چند دقیقه یک‌بار (هر بار کاربری کاری بکنه) اتفاق بیفته، نه یک لحظه‌ی نادر، این هشدار
    برای کاربر واقعی شد. راه‌حل: دقیقاً همون قدمِ اول sync_data_dir (پاک‌کردنِ index.lockِ
    جامونده، git add، و commit فقط اگه واقعاً تغییرِ محلی باشه) رو اینجا هم قبل از pull انجام
    می‌دیم - یعنی این تغییراتِ محلی از حالتِ «کثیفِ کامیت‌نشده» به «تمیزِ کامیت‌شده‌ی هنوز-پوش‌نشده»
    می‌رن (بدون push کردنشون همین‌جا - چرخه‌ی دوره‌ای بعدی طبق معمول پوششون می‌کنه)، و
    `git pull --rebase` روی یک درختِ تمیز دیگه به این دلیل رد نمی‌شه.

    best-effort و بی‌خطر: اگه pull باز هم شکست بخوره (یک تعارضِ محتواییِ واقعی یا مشکل شبکه)،
    rebase را abort می‌کنه و False برمی‌گردونه - فراخوان باید در این حالت همچنان با هر چی روی
    دیسک هست ادامه بده (بهتر از قدیمی، بهتر از هیچی)، نه اینکه کاربر را با خطا معطل کنه."""
    def run(args):
        return _run_git(args, cwd=repo_dir)
    ...
    if add_res.returncode == 0:
        diff_res = run(["git", "diff", "--cached", "--quiet"])
        if diff_res.returncode != 0:  # یعنی واقعاً تغییرِ محلیِ منتظرِ کامیت هست
            run(["git", "commit", "-m", commit_message])
            # اگه همین commit هم شکست بخوره، تلاشِ pull پایین همچنان انجام می‌شه - در
            # بدترین حالت دقیقاً همون رفتارِ قبلی (ممکنه به‌خاطر درختِ کثیف رد بشه)، نه بدتر.
